...
- 带 task ID 的 body 使用 `contextReadSeeker`，保留重试 rewind 能力；签名前的原始文件 hash 不触发任务字节累计，进度只反映实际请求体发送。`objects_resume_test.go` 在服务器尚未响应时钉住两条路径均为 `N/N`，同时校验请求体及 `X-Amz-Content-Sha256`。取舍见[决策记录](../notes/implemented/bug-fix/2026-09-10-s3-tracked-upload-payload-hash.md)。
