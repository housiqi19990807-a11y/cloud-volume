...
- 顶层 tab 页共享 chrome：[mobile_page_chrome](../../../lib/widgets/mobile_page_chrome.dart) 提供 `MobilePageHeader`（稳定大标题 + 副标题 + 单一 48dp 动作入口）与 `showMobileActionSheet`（48dp 底部动作抽屉，文件管理同款实现）。账号/任务/回收站/设置页已按 2026-09-04 批次对齐该基线：SafeArea(bottom:false) + 16dp 边距、23sp 标题 + 13sp 副标题、触控目标 ≥48dp（含账号卡片动作、任务行内图标与选择控件、回收站 compact trailing、设置底部导航上移/下移钮）；设置页详情↔索引的系统 Back 链由 `MobileSettingsNavigation`（shell 持有）承接，先于 tab 历史消费。分享管理页与同步任务页在 Android 无底栏入口（不在 `kMobileBottomBarPool`），未做小屏适配；进入底栏池前必须先补。2026-09-15 第一批把五个底栏页（文件/账号/任务/回收站/设置）及文件管理页内桶回收站视图统一到无边框基线（见上），第二批把任务页与回收站页头部的未选中态动作收进右上角单一 48dp 入口 + 底部抽屉（批处理运行中入口图标变 spinner、无可用动作时入口隐藏），第三批把两页选中态改为 [mobile_selection_action_bar](../../../lib/widgets/mobile_selection_action_bar.dart) 底部动作条：顶行「取消 / 已选中 N 个 X / 全选」+ 发丝线下方一行批量动作（48dp、批处理运行中禁用、底部避让安全区、AnimatedSize 进出），标题槽在选中态保持常显；决策分别见 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-borderless-baseline.md)、[Agent Note](../notes/implemented/architecture/2026-09-15-mobile-header-action-sheet.md) 与 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-selection-action-bar.md)。存量修复：`GlobalTrashBrowser` 行的 `onDoubleTap`（桌面双击恢复）在 Android 置空——非空双击识别器会使行单击的选中被立即取消（嵌套 tap 双触发），移动端恢复走行尾图标与动作条；行仍保留右键/长按菜单包装（对象行同款，不影响触摸点击）。

**Known P2/P3 (review 2026-09-04):** P2 任务行最坏组合（spinner+状态徽标+取消+展开，行内动作 Android 48dp 化后固定宽约 343px）在 320dp 屏扣 16dp 边距会溢出约 23px，360dp 无碍——真机统一测试时验证，必要时行内动作收进溢出菜单。P2 账号卡三按钮 320dp 下「桶管理」13sp 标签可用宽不足可能折行，可缩短标签或 maxLines 取舍。P3 账号卡 deleteProfile 动作沿用「退出」文案（与既有 toast 一致），破坏性语义弱化待产品定夺。（原 P3「回收站用例名 swap-title 未真正驱动选中态、任务页已选 N 项切换缺断言」已由 2026-09-15 选中动作条批次解决：行选中已可驱动且标题切换移除。）
