...
- 顶层 tab 页共享 chrome：[mobile_page_chrome](../../../lib/widgets/mobile_page_chrome.dart) 提供 `MobilePageHeader`（稳定大标题 + 副标题 + 单一 48dp 动作入口）与 `showMobileActionSheet`（48dp 底部动作抽屉，文件管理同款实现）。账号/任务/回收站/设置页已按 2026-09-04 批次对齐该基线：SafeArea(bottom:false) + 16dp 边距、23sp 标题 + 13sp 副标题、触控目标 ≥48dp（含账号卡片动作、任务行内图标与选择控件、回收站 compact trailing、设置底部导航上移/下移钮）；设置页详情↔索引的系统 Back 链由 `MobileSettingsNavigation`（shell 持有）承接，先于 tab 历史消费。分享管理页与同步任务页在 Android 无底栏入口（不在 `kMobileBottomBarPool`），未做小屏适配；进入底栏池前必须先补。2026-09-15 第一批把五个底栏页（文件/账号/任务/回收站/设置）及文件管理页内桶回收站视图统一到无边框基线（见上），第二批把任务页与回收站页的页面级动作收进右上角单一 48dp 入口 + 底部抽屉（批处理运行中入口图标变 spinner、无可用动作时入口隐藏）；第三批曾落地选中态底部动作条，同日第四批按用户反馈改为**行级 `…` 抽屉模型（binding，文件管理契约）**并删除该组件：行的全部动作收在行尾 48dp `…` 入口打开的共享底部抽屉——无选中时为该行动作（回收站：恢复/彻底删除；任务：立即执行/取消任务/重试/清理历史），本行已选中且选中数 >1 时为当前选择的批量动作（含「取消选择」，抽屉标题「已选 N 个 X」；单个选中的批量等价于行自身动作，走行级分支——与文件管理对象行同款触发条件）；两页无选中动作条、无全选入口、无列表头（共 N 项/全选/速度一并移除），搜索框与列表按文件管理页间距紧贴（头部后 14dp、筛选/搜索后 12dp）；页面级动作（任务页立即同步/清理全部历史、回收站页刷新/清空回收站）保持右上角单一入口，选中态不隐藏。行点击切换选中（回收站行 `onDoubleTap` 需置空，否则嵌套 tap 双触发抵消单击选中）。决策见 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-borderless-baseline.md)、[Agent Note](../notes/implemented/architecture/2026-09-15-mobile-header-action-sheet.md)、[Agent Note](../notes/implemented/architecture/2026-09-15-mobile-selection-action-bar.md) 与 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-row-overflow-drawer.md)。同日另一批把任务行格式与文件/回收站行统一：全宽发丝分隔线、标题 14sp w500、Android 行尾单一 `…` 且任务明细展开为抽屉首项（批量抽屉同携；桌面保留内联图标与明细 chevron）。
...
**Known P2/P3 (review 2026-09-15 行级抽屉批次):** P3 批处理运行中任务行 `…` 仍呈可用态、点击静默无操作（页面入口有 spinner 全局提示；回收站忙行则已禁用）——待统一。P3 本批 `dart format` 漂移未全量修复（仓库无 format 门禁，仅修新增漂移点）。读屏语义已带条目名（`X 的更多操作`），与页面入口可区分。
