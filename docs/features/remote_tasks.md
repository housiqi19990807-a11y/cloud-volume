...
- `lib/pages/transfers_page.dart` / `lib/pages/transfers_page_remote.dart` / `lib/pages/transfers_page_remote_overflow.dart` — 按 活跃/等待/关注/历史 分组显示有效 `RemoteTask` 操作;原始 journal 事件与物理阶段从 `RemoteTaskRow` 展开。legacy `TransferTaskRow` 是无引用兼容代码,不是显示路径。桌面保留上游头部(标题常显、队列标签、分组头、共 N 项/全选/速度列表头)与内联批量按钮、带边框的列表/空态 `ShadCard`。Android(`_androidCompactQueueHeader`,`defaultTargetPlatform == TargetPlatform.android`)窄屏适配:隐藏队列标签行、列表分组头与列表头,标题常显 23sp+副标题,搜索框与列表按文件管理页间距紧贴(14/12dp);行内取消/重试/立即执行图标收进行尾 `…` 的底部抽屉(`RemoteTaskOverflow` 由 `transfers_page_remote_overflow.dart` 装配:无选中=行级 立即执行/取消任务/重试/清理历史,有选中=批量动作,抽屉标题「已选 N 个任务」),行内保留状态徽标、spinner 与明细展开;页面级动作(立即同步/清理全部历史)在右上角单一入口抽屉,批处理运行中入口变 spinner、行抽屉动作集为空,列表与空态按 mobile_ui 无边框基线直接落在页面背景上(桌面窄窗仍保留卡片)。`lib/pages/global_trash_page_view.dart` 选中态同样走行级 `…` 抽屉。`transfers_page_remote_filters.dart` 在 Android 把搜索框叠在两个筛选下拉上。`remote_task_store_polling.dart` 在活跃轮询报告历史增长时自动重载首页;Android **同时**渲染列表内 `_RemoteHistoryPager` 作为显式加载更多入口——自动重载只刷新首页,完成的下载无需翻页即呈现(桌面保持仅 cursor 失效语义)。行级抽屉模型决策见 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-row-overflow-drawer.md)。
