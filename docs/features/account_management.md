...
- `lib/widgets/cloud_storage_account_list.dart` — 列表/卡双视图装配与 `enum AccountStatus { checking, ok, error, disabled }`。`_AccountActions`(列表)与 `_AccountCard`(网格/移动)等展示件在 part 文件 `lib/widgets/cloud_storage_account_card.dart`(含 `_AccountStatusChip`、`_AccountActionButton` 的 dense/48dp 双变体、`_AccountTableHeader`、拖拽把手)。两处都显示 `ShadSwitch`(value = `!profile.disabled`),切换调 `onToggleDisabled`;禁用账号标题加「（已禁用）」后缀。Android 移动卡:开关行 48dp 高,三个动作按钮(桶管理/编辑/退出)为 48dp 触控的非 dense 变体,账号块为无边框 `Padding`(mobile_ui 无边框基线,块间发丝分隔线由 `_buildMobileList` 提供),桌面网格/表格保持 `ShadCard`;`_AccountStatusChip` 错误详情经 `Semantics` 暴露(触屏无 hover tooltip),桌面保留 Tooltip。
