...
- 回收站 UI:文件管理头部回收站图标(`lib/widgets/file_manager_action_bar.dart`,由 `_activeBucketTrashEnabled` 门控)、每桶回收站浏览器 `file_manager_trash_browser.dart`、全局页 `lib/pages/global_trash_page.dart` + `global_trash_browser.dart`、设置区 `settings_trash_section.dart`、侧栏入口 `main_layout_page.dart`。两个回收站浏览器的列表在 Android 直接落在页面背景上(mobile_ui 无边框基线,与桶/对象移动列表一致),桌面(含窄窗 compact 分支)保持带边框 `ShadCard`。`GlobalTrashBrowser` 行的 `onDoubleTap`(桌面双击恢复)在 Android 置空——非空双击识别器会使行单击的选中被嵌套 tap 双触发抵消;全局回收站页 Android 行的全部动作(无选中=恢复/彻底删除,有选中=批量,文件管理契约)收进行尾 `…` 的底部抽屉,页面级动作(刷新/清空)在右上角入口抽屉,`GlobalTrashHeaderActions` 变桌面专属,决策见 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-row-overflow-drawer.md)。
