...
- 回收站 UI:文件管理头部回收站图标(`lib/widgets/file_manager_action_bar.dart`,由 `_activeBucketTrashEnabled` 门控)、每桶回收站浏览器 `file_manager_trash_browser.dart`、全局页 `lib/pages/global_trash_page.dart` + `global_trash_browser.dart`、设置区 `settings_trash_section.dart`、侧栏入口 `main_layout_page.dart`。两个回收站浏览器的列表在 Android 直接落在页面背景上(mobile_ui 无边框基线,与桶/对象移动列表一致),桌面(含窄窗 compact 分支)保持带边框 `ShadCard`。`GlobalTrashBrowser` 行的 `onDoubleTap`(桌面双击恢复)在 Android 置空——非空双击识别器会使行单击的选中被嵌套 tap 双触发抵消。两个回收站表面在 Android 都走两态选择模型(mobile_ui binding):浏览态行尾选择圆点、行点击进入选中(回收站行无浏览态主操作);选中态头部变形「取消/已选中 N 个文件/全选」+ 底部动作条(单选=详情(`showMobileDetailSheet` 展示 名称/类型/大小/删除时间/原路径/所属桶)+恢复/彻底删除,多选=批量恢复/彻底删除;桶内回收站的批量在 `file_manager_page_mobile_selection.dart`,mutation 发起即退出选中态;全局页批量沿用 `_runBusy` 的先清选择语义)。页面级动作(刷新/清空)保持右上角入口抽屉(一度按「已被覆盖」移除,次日按用户裁决恢复);桌面 `GlobalTrashHeaderActions` 保留刷新/批量/清空,决策见 [Agent Note](../notes/implemented/architecture/2026-09-15-mobile-two-state-selection.md)。
