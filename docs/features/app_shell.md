...
- `lib/pages/mobile_file_manager_page.dart` / `mobile_file_manager_presentation.dart` — Android 无状态入口把 `mobileNavigation` 与 `MobileFileManagerNavigation` 交给 `FileManagerWorkspace`；独立呈现层渲染与桌面导航同名的「文件管理 + 桌面同款首页说明/进入后的当前位置」页头、常驻搜索和右上 48dp `+` 操作入口。文件/目录位置的入口经 `showAppModal` 底部抽屉列出新建目录和上传（不可写/加载中隐藏）；全局回收站经 `SidebarItem.trash` 独立导航进入。回收站视图的入口列出返回文件与可用时的清空操作（一度按「已被覆盖」移除,次日按用户裁决恢复）。二级位置的 48dp 顶栏 Back 与系统 Back 共用位置栈，且在加载、错误状态也可用。
...
- `lib/widgets/file_manager_object_browser.dart` / `file_manager_object_browser_helpers.dart` / `file_manager_object_browser_mobile.dart` — 桌面对象浏览继续使用表头、卡片、网格和 `DesktopContextMenuRegion`;Android 固定为 `FileListTile(compact: true)` 行，隐藏这些桌面 chrome 与本地挂载同步徽标。对象行遵循两态选择模型（mobile_ui binding）：浏览态行尾是 48dp 选择圆点（`MobileRowSelectDot`）、无行首控件，行点击=打开、长按=选中；选中态行首 48dp 勾选控件接管、行点击切换选中，头部变形与底部动作条由页面装配（`file_manager_page_mobile_selection.dart`：单选=详情（`showMobileDetailSheet` 文件详情 sheet）+打开/下载/分享 + 复制/移动/重命名/删除（两排全量展示），多选=下载/复制/移动/删除，readOnly/目录下载门控与桌面契约一致；桶内回收站共用同一模型）。桌面共享的行 helper 保持对象打开、选择和删除态逻辑一致。桶级动作由 `file_manager_bucket_browser_mobile.dart` 进入 Android 底部抽屉（桶不可选、桶行 `…` 保留），桌面继续使用上下文菜单。
...
**Known P2/P3 (review 2026-09-02, resolved):** P2 页面级 `+` 抽屉只保留文件写入操作（不含「回收站」或「打开回收站」）；全局回收站由独立底栏入口承载，桶行 `…` 保留其桶级「打开回收站」，回归锁定两类入口不混用。
