...
- `cloud_files_provider_transfer_windows.go` / `cloud_files_callbacks_windows.go` — 每个 `FETCH_PLACEHOLDERS` callback 都以自己的 callback info 经 `CfExecute(TRANSFER_PLACEHOLDERS)` 交付 metadata 的实际子项；单批完整快照传 `TotalCount == Count == len(items)`、`STOP_ON_ERROR | DISABLE_ON_DEMAND_POPULATION`。C heap create-info 数组及 UTF-16/identity 缓冲只活到同步调用返回，随后要求 `EntriesProcessed` 全量且每条结果成功才记录 projection。TTL/inflight 只缓存不可变描述符，等待者和缓存命中仍各自 transfer，绝不能回复零数组。初始 root 由 `CfCreatePlaceholders` 投影，注册时禁用 root 按需枚举以避免同名双创建。启动投影由 `PopulatePlaceholders` 递归 `populateChildDirectories` 深度优先投影每层子目录：无 Explorer 参与时占位目录不会触发 `FETCH_PLACEHOLDERS` 回调（root 回调也不会来），只投影一层会让嵌套内容永不出现；callback 路径仍按需服务 Explorer 刷新。设计理由见 [callback transfer 决策记录](../notes/implemented/bug-fix/2026-09-10-cloud-files-callback-placeholder-transfer.md)。
- `cloud_files_watcher_windows.go` / `cloud_files_watcher_mutations_windows.go` / `cloud_files_watcher_state_windows.go` / `backend_windows_cloud_files_cgo.go` — metadata 挂载中，普通 NTFS 文件的 `Rename(old)+Create(new)`（或 `Remove(old)+Create(new)`）fsnotify 序列按目录内唯一 size/mtime 指纹配对，普通 `Remove` 也经 `deletePath` 写入删除 journal。投影身份与待水合状态分别追踪：已水合占位符仍由 CFAPI delete completion 独占删除 admission；callback 先到时短暂忽略尾随 watcher 事件，watcher 先到时保留 Desired 与 kind 等 callback，避免重复 journal。write/mkdir/delete/rename admission 共用 FIFO 后台队列；写入在事件线程只打开带 delete-share 的源句柄，后续物理 rename 不会令排队读取丢失，关闭前排空已接收项，避免批量 Explorer 改名阻塞 fsnotify 读取。rename 只在 worker admission 成功后 Rebase watcher 状态；普通文件 admission 失败时从已移动的新路径写入内容、再删除仍存在的旧 Desired 路径，不能因异步化吞掉唯一 Create fallback。pending rename 记录只覆盖短配对窗口，`fallbackRenameHandled` 使用独立 60 秒 TTL 抑制晚到 CFAPI completion。legacy（无 metadata write path）挂载保持旧行为。回归锚点 `cloud_files_watcher_windows_test.go`、`cloud_files_watcher_mutations_windows_test.go`；设计理由见 [删除与改名 journal 决策记录](../notes/implemented/bug-fix/2026-09-10-cloud-files-watcher-mutation-journal.md)。
...
- `go/mount/metadata_write.go` / `metadata_write_source_windows.go` — stage 写源以 `GENERIC_READ` 和 `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE` 打开，保留 Go 的普通/UNC 长路径规范化与 `FILE_FLAG_BACKUP_SEMANTICS`。watcher 在排队前打开句柄，FIFO worker 在同一 `writebackMu` 下从 `Stat` 持有到 `Service.WritePath` 返回；父 revision 被 metadata worker 并发推进时 rewind 并重试 `ErrStaleCursor`。Cloud Files 的物理 rename 可在排队读取前完成而不丢源内容，Desired rename 仍排在旧路径 write journal 之后；不得恢复默认 `os.Open` 的无 delete share。
...
**Known P2/P3 (review 2026-09-10, 独立复核):** `writebackMu` 跨完整读取+落盘,大文件写入期间全局串行化挂载操作(正确性换简单性的既有取舍)。P3:rename 配对仅限同目录,跨目录移动退化为上传+旧 key 残留。
