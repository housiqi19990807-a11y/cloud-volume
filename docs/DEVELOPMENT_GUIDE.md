...
- **Android:** Windows 出包用 `scripts/build_android.ps1`(先建 ARM64 桥接 `libremote_storage_bridge.so` 再出 release APK),环境引导用 `scripts/setup_android_dev.ps1`。macOS 调测:一次 `make android-setup` 引导 JDK/SDK/NDK/模拟器与 `cloud-volume` AVD,日常 `make android-run`(双 ABI 桥 + 无设备时启动模拟器 + `flutter run`)。细则见 [android_dev](features/android_dev.md)。**移动端迭代工作流(用户要求):每批移动端改动验证通过并提交后,当场构建新的 arm64 release APK(`scripts/build_android_bridge.sh --abi arm64-v8a` + `flutter build apk --release --split-per-abi --dart-define=APP_VERSION_LABEL=dev`,产物取 `build/app/outputs/flutter-apk/app-arm64-v8a-release.apk`)供真机安装,不留给用户手动触发。**
