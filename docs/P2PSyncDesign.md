...
3. **D2：局域网内容直传（进行中）。** B 端读文件时，先查局域网 peer 有无相同 S3 ETag 的完整副本。有则通过 QUIC 分块并行拉取，拉完后校验每块 HMAC 与远端 HEAD 的 size/ETag。没有稳定 ETag 的后端直接回退远端对象存储；不扫描完整文件计算 hash。分块大小可在设置页面配置（1-64 MB，默认 4 MB）。
...
- 内容直传校验失败（HMAC 不匹配 / ETag 过期）自动丢弃，回退远端对象存储。
