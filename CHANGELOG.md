...
- macOS WebDAV 挂载根目录现在通过 RFC 4331 返回 `quota-available-bytes` 与 `quota-used-bytes`，优先使用桶自定义容量并复用上游实际配额/已用量，让 `df` 不再在已有容量信息时显示 `0/0`；配额响应做 30 秒缓存，避免 Finder 的重复 `PROPFIND` 持续查询远端。
