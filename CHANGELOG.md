...
- Android 任务队列与全局回收站页的多选交互改为底部动作条：选中条目后在列表下方弹出动作条，顶行为「取消 / 已选中 N 个 / 全选」，下一行为批量动作（任务：立即执行、取消任务、清理历史；回收站：恢复、彻底删除），一键直达；页面标题在选中态保持常显不再切换，右上角入口只承载未选中态动作（立即同步/清理全部历史、刷新/清空回收站）。桌面端不变。
