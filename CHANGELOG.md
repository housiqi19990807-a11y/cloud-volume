...
- macOS 关闭确认弹窗现在默认按钮回到“退出云卷”，避免点击关闭后默认执行最小化到托盘。
- WebDAV 目录权限探测现在会把 PROPFIND / OPTIONS 的状态码、Allow 头、解析到的 privilege 和最终判定写入 bridge 日志，便于排查应为只读却被放行的目录。
- WebDAV 目录权限探测在 ACL 缺失时会继续按当前目录的 OPTIONS Allow 判断；如果 Allow 明确没有 PUT / MKCOL，会把当前目录判定为只读。
...
- 桌面端图片预览现在会打开独立子窗口，使用更大的 1040x760 默认视图并支持缩放查看；不支持子窗口的平台仍回退到原预览对话框。
