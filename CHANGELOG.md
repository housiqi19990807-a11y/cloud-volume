...
- 局域网 P2P 同步（D1）：同局域网内登录同一账号的多台设备通过 mDNS 自动发现（`hashicorp/mdns`），无需配对或扫码。写入完成后通过 QUIC（`quic-go`）即时广播签名事件（Ed25519），接收端立即刷新挂载目录缓存，不再依赖 5 秒远端轮询。可在「设置 → 局域网同步」开关 P2P、配置内容直传分块大小（默认 4 MB，范围 1–64 MB）、查看已发现设备列表。远端对象存储始终是权威来源，P2P 仅用于通知和读加速，校验失败自动回退远端。
