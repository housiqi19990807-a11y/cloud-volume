...
- Android 任务队列与全局回收站页的可选动作不再内联暴露在页面头部：队列级/批量操作（立即同步、清理全部历史、选中清理历史、批量恢复/彻底删除、清空回收站、刷新）统一收进右上角带语义的 48dp 入口，点击打开底部动作抽屉，与文件管理页一致；批量操作执行中入口图标变为 spinner，没有可用动作时入口隐藏。桌面端头部按钮与布局不变。
