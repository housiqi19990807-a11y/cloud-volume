...
- 设置页：通用设置内的每个区块（应用更新、网络代理、外观、下载设置、缓存设置、显示设置、同步设置、回收站、WebDAV 凭据、账号重置、配置管理）拆分为左侧导航栏中的独立 tab，点击即可切换到对应配置卡片，不再需要在单个长页面里滚动查找。Windows 设置和关于也各自拆为独立 tab，左侧导航栏用分组标题（通用 / Windows / 关于）分隔。
