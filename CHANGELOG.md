...
- macOS WebDAV 挂载根目录现在通过 RFC 4331 返回 `quota-available-bytes` 与 `quota-used-bytes`，优先使用桶自定义容量并复用上游实际配额/已用量，让 `df` 不再在已有容量信息时显示 `0/0`；挂载握手前会限时 2 秒预取配额，使 macOS 首次 `PROPFIND` 即可读取容量并避免缓存成 `0/0`，慢速 SFTP 配额探测超时后仍继续挂载并在后台刷新。若 `osascript` 超时但卷已实际出现在系统 mount 表中，应用会接管该挂载并保持本地 WebDAV 服务，不再误报失败后留下断开后端的死挂载；清理仍在 mount 表中的死卷时也不再调用可能阻塞几十秒的 `os.Stat`，会直接进入 `diskutil` 回退。
