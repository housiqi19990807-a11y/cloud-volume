...
- Android 文件管理（对象/桶内回收站）、全局回收站与任务队列统一改为两态选择模型（参照百度网盘）：浏览态列表行去掉行首常驻复选框，行尾是一个安静的选择圆点，行点击保持主操作（文件打开、回收站/任务进入选中），长按也可进入选中；进入选中态后顶部标题区变形为「取消 / 已选中 N 个 X / 全选（全选中显示取消全选）」，行首出现勾选控件，底部导航栏整体隐藏、由全宽贴底的动作条取代——动作全部直接可见（≤4 个单排，更多时两排均衡分列，无「更多」二级入口；标签用短动词、计数只在头部），文件、回收站与任务的单选首项为「详情」，打开底部弹层展示名称/类型/大小/时间/路径/所属桶（任务为操作/状态/路径/阶段等）等系统已有元信息；任务明细不再行内展开。每行 `…` 抽屉随之移除（桶行 `…` 保留，桶不可选）；页面级动作（新建目录/上传、回收站刷新/清空、任务操作）保持右上角单一入口、选中态隐藏。文件管理页系统 Back 先清空选择再回退位置；回收站动作发起即退出选中态。桌面端（含窄窗口）渲染不变。整套两态选择与列表布局已在 HyperOS 迷你小窗级极窄宽度（Xiaomi 13 Pro 实测，窄于最窄真机）验证通过。
- Android 账号页与设置页的卡片容器恢复（无边框平铺风格收敛为文件/回收站/任务列表页专属）：账号块、设置索引分组与详情分区回到带边框卡片；其余底栏页维持无边框列表基线，桌面端不变。
- Android 任务队列的行格式与文件管理/回收站行统一：分隔线改为同规格全宽发丝线，标题字重对齐（14sp/w500），行尾仅保留单个「…」入口，任务明细展开移入该抽屉的首个动作；桌面端保留原有内联按钮与明细箭头。
- Android 任务队列与全局回收站页的行级与批量动作统一收进行尾 `…` 的底部动作抽屉（文件管理页同款）：无选中时为该行动作（回收站：恢复/彻底删除；任务：立即执行/取消任务/重试/清理历史），选中行在选中数大于一时为当前选择的批量动作（含「取消选择」，与文件管理页同款触发条件）；不再提供全选与选中动作条，列表头（共 N 项/全选/速度）在 Android 移除，搜索框与列表按文件管理页间距（14/12dp）紧贴，页面级动作（立即同步/清理全部历史、刷新/清空回收站）保持在右上角入口且选中态不隐藏。桌面端不变。
- 修复 Android 全局回收站的行点击无法选中：行上注册的桌面双击恢复手势会使单次点击的选中被立即取消；移动端不再注册双击恢复，恢复操作走行尾图标与选中动作条，桌面双击恢复保留。
- Android 任务队列与全局回收站页的可选动作不再内联暴露在页面头部：未选中态的队列级操作（立即同步、清理全部历史、刷新、清空回收站）统一收进右上角带语义的 48dp 入口，点击打开底部动作抽屉，与文件管理页一致；批量操作执行中入口图标变为 spinner，没有可用动作时入口隐藏。桌面端头部按钮与布局不变。
- Android 五个底栏页（文件/账号/任务/回收站/设置）及文件管理页内的桶回收站视图统一为无边框平铺风格：列表、账号块、设置索引分组与详情分区、空态直接落在页面背景上，行/块间用与文件列表同规格的发丝分隔线；带边框的卡片容器仅保留给桌面（含桌面窄窗口）。桌面端渲染不变。
- 修复 Web 和内嵌 Web 的 full CLI 构建失败：浏览器侧 `DesktopFileTransferService` 补齐拖拽文件路径的跨平台方法契约，并保持浏览器不读取本地文件路径。
...
- Android 文件管理改为独立移动端呈现层：使用「文件 + 当前位置」页头、常驻搜索和固定紧凑列表，桶的行内设置图标移除，桶、文件和目录的行尾 `…` 均以底部抽屉提供适用动作；对象还支持长按打开抽屉、多选 48dp 选择区、只读目录隐藏写入动作和触底续载后保持新页可见。挂载相关入口继续完全隐藏。桌面仍保留原有面包屑、操作栏、网格与右键菜单，两个平台共用数据加载、mutation 和文件位置返回栈。首页 route 明确设置 Android 浅色系统栏样式，恢复状态栏时间、信号和电量的深色图标显示。
- Android 应用 ID、namespace 与启动 Activity 包名改为 `com.cloud.volume`。该变更会被系统视为新应用，旧 `cn.ihep.cloudvolume.remote_storage` 可继续并存；切换前请使用远端配置备份保存账号配置，并在新包首启时还原，未备份的账号需重新配置，旧包的缓存与任务不会自动迁移。
- Android 应用内拟态框统一改为从底部上移的内容自适应抽屉：短确认框按内容高度贴底，长编辑器/目录选择器/预览最多占状态栏下、软键盘后可用高度的 90%，始终保留顶部暗层而不再全屏留白。背景延伸到导航栏下、内容避让安全区，进入/退出为上移/下移动画；横屏 + 软键盘的低高度场景可整体滚动，窄屏连接字段、深层面包屑及所有表单 footer 按钮可分行。桌面与 Web 继续保持有限宽居中弹窗，焦点环仍预留绘制边距。
- 修复配置备份与 Android 首启「从备份存储还原」目录弹窗:手机宽度下四个底部按钮会按组换行,目录行也不再被桌面固定元数据列挤成省略号;WebDAV、百度网盘、FTP、SFTP 的唯一合成桶显示为「备份存储」并直接进入,设置页已保存路径同样使用别名,真实桶 identity 保持不变,S3 继续显示真实桶列表。空或失效的初始 bucket 不再把 `cloud-volume-config-backups` 强套到首个存储并触发 `file does not exist`;返回有效目录会清除旧错误,远端快照为空时显示明确空状态而不永久转圈。
- 修复 Android 点击还原配置备份时可能使进程因 bbolt `page already freed` 终止：并发 Flutter FFI 调用现在复用一个进程内 `config.db` 句柄，避免独立句柄在同一数据库上竞争空闲页状态；切换应用私有数据目录会等待当前配置调用结束。
...
- 任务队列历史分页不再固定在卡片底部：`加载下一页（还剩 N 条）` 与 `历史已显示 N / 总数` 移入列表成为最后一项，只有滚动到最后一行时才出现；加载中显示为列表末尾居中的「正在加载历史…」小转圈行，未滚到底部时不再常驻底部提示。历史-only 队列仍保证分页入口可达。
- 统一各页面加载效果，以文件管理页为规范并写入 AGENTS.md：所有加载 spinner 一律使用 `AppLoadingIndicator`（弧形样式 + 轨道环），禁止原生 `CircularProgressIndicator`；按场景分档（页面主体 22/2.4 带阶段文案、按钮内联 14/2、对话框主体 18/2、密集行内 12–15、大对话框 hero 42/3）。任务队列首次加载历史时显示标准居中加载视图「正在加载任务…」；任务队列按钮/任务行、账号向导、存储桶可见性、目录选择器、配置备份对话框与设置区、P2P 设备搜索、文件预览、桌面子窗口共 14 处替换为同款 spinner；线性进度条仅保留用于有确定进度的进度条。
- 重新设计任务队列的任务行视觉：操作类型图标改为彩色圆角底衬、状态徽标更精致（浅底描边胶囊），活动状态统一用一枚小转圈标记，操作按钮紧随徽标右侧；展开明细与分隔线对齐标题文本列（左侧 80px），明细面板使用类型色左侧强调条 + 键值行（依赖/错误分色），事件行改用等宽小字。样式查表（状态/类型 → 标签、颜色、图标、副标题、速度汇总）拆分到 `remote_task_style_helpers.dart` 供任务页与侧边栏共用。
- Android：任务队列页窄屏适配——隐藏队列标签行与列表分组标题（状态下拉已覆盖同样筛选）；选中任务后标题槽显示「已选 N 项」且只保留「清理历史」按钮（立即执行/取消由行内小图标承担）；完成的下载/上传任务通过历史页自动呈现，无需重进页面。回收站页选中条目后同样隐藏标题文字为操作区腾出宽度。以上行为仅作用于 Android，桌面端保持原有布局。
- 任务队列新增队列级「立即同步」：一次性唤醒当前范围内所有 metadata pending 操作，按钮内显示 loading；它只跳过静默/重试等待，worker 仍逐项执行并保持同 inode、父目录、重命名等依赖顺序。
- 修复任务队列历史总数与已显示行数不一致却找不到入口：少于 100 条历史会完整显示，更多历史使用明确分页（分页入口位于列表末尾）；WebSocket 继续即时刷新活跃任务与总数，每次历史总数增长都会作废旧 offset cursor、从首页重新打开分页，避免连续完成任务时跳过最新行。
- 优化任务队列「清理全部历史」：metadata journal 现在按序号反向单次扫描，在线性时间内保留被后续未完成操作依赖的记录，不再对每条历史重复全表扫描；清理全部或已选历史期间，按钮会显示转圈与「正在清理…」。
- 修复任务页切换时的 `setState() or markNeedsBuild() called during build`：历史自动加载现在在帧结束后启动，侧边栏传输动画也会延后同步，避免 `ScaleTransition` 在父树构建中被标脏。任务页新增始终按后端历史总数显示的「清理全部历史」按钮，可不选中当前已加载页就清理全部可清理历史。
- 任务队列首次切入现在会自动加载最多 100 条历史；小队列不会再要求先点“加载更多历史”，大队列首次加载会显示标准加载视图。清理历史后会使旧分页 cursor 失效，避免删除后又从过期位置加载出错位的下一批记录。
- 修复任务队列的“后端已空、页面仍显示历史 200 条”错位：队列计数和 Total 现在始终以完整 Go 任务集（含历史）计算，active-only 轮询只影响返回行；桌面与 Web 都能返回超过 100 条的 active 行。历史-only 队列可从空页主动加载，失败/冲突保留在重试队列而非被当作可清理历史；任务组缓存会随 namespace 打开/关闭及状态变化失效，避免短暂返回 0。Dart 本地执行兼容任务不再在完成、取消、失败或重启后伪装成持久历史，避免计数为 0 时残留历史列表。
- 新增本地任务诊断端点与推送通知：`make run` 默认启动仅 loopback 的 `CV_DEBUG_ADDR`，可查询 metadata/runtime 快照并通过 WebSocket 触发即时刷新；动态 namespace、取消/重试/验证/恢复/清理等状态变化都会通知订阅者，关闭页面会同时关闭客户端和服务端监听。

- 修复任务列表分页把进行中任务挤出第一页、导致文件管理页不再随同步完成自动刷新的问题：桌面与 Web 的任务排序统一改为“未完成任务优先（最新在前），历史随后”，第一页始终包含所有进行中/失败/等待任务；轮询默认不再拉取历史，历史仅在点击“加载更多历史”时按页加载。任务页顶部新增队列切换标签（全部/进行中/等待中/失败/历史），直接显示各队列数量并可一键切换。
