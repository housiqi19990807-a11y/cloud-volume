...
- Windows Cloud Files watcher recovery now re-registers placeholder directory watches after the placeholder is actually created on disk, recursively harvests freshly copied local directory trees after their parent folder appears, and serializes watcher `Add/Remove/Close` calls during shutdown, which fixes the cases where copying a folder into an already opened nested directory such as `bakcuptest/test/test2` produced no upload task, hid the new files from the local-first file manager, or left later unmounts stuck around `watcher-close-start`.
