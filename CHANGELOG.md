...
- 配置备份：设置页新增加密远端配置备份。顶部「开启备份」开关控制功能启停；开启后才显示备份目标设置，目标可选已有账号或按需弹出的独立备份存储（不显示在账号列表）。保存位置改为单个远程目录选择器一次选定 bucket 与目录，移除了两个手动输入框。新增「配置备份密钥」按钮说明密钥派生方式与凭证变更影响。备份历史通过可点击摘要卡片打开拟态框查看，并支持二次确认还原账号、全局代理和显示排序。
