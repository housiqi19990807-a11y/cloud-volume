...
## Unreleased
- 文件管理配额：WebDAV 根目录现在通过 RFC 4331 的 `quota-available-bytes` / `quota-used-bytes` 获取真实总容量，百度网盘通过 xpan Quota API 获取账号总容量；桶列表在未设置自定义配额时显示服务端配额，查询不受支持或失败时降级为 `--`，不影响文件浏览。
