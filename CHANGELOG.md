...
- 配置备份：设置页新增加密远端配置备份。加密密钥改为从用户自设的备份密码派生（不再依赖连接凭证，换机器、换 endpoint 都能解密），开启备份时必须设置密码。新机器首次启动可从备份存储还原。顶部「开启备份」开关控制功能启停；开启后才显示备份存储设置，目标可选已有账号或走简化流程配置独立备份存储（仅选协议+连接凭证，不显示在账号列表）。保存位置改为单个远程目录选择器一次选定 bucket 与目录，移除了两个手动输入框。备份历史通过可点击摘要卡片打开拟态框查看，并支持二次确认还原账号、全局代理和显示排序。
