...
- 新增文件同步功能：可指定本地目录定期同步到远端桶目录，支持仅上传、仅下载和双向同步。双向同步通过本地索引加三方比对（本地快照 / 远端列表 / 持久化索引）识别新增、修改、删除与重命名操作，冲突按较新、本地优先、远端优先或跳过策略处理；正在写入的热数据会等到静默指定秒数后才纳入同步，避免频繁操作远端。同步任务复用现有任务队列，新增 sync_upload / sync_download / sync_delete / sync_rename 任务类型。系统设置页新增独立的文件同步子 Tab 管理同步配置，主导航新增文件同步页面展示同步进度与状态。同步索引采用 bbolt KV 存储，按 key 单条读写，内存占用恒定且不随文件数量增长。
