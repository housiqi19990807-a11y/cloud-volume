...
- 新增百度网盘 OpenAPI 上游：账号管理与首次初始化现在支持“百度网盘”类型，桌面端会按百度要求使用 `oob` 模式打开浏览器授权页，用户把网页显示的授权码手动粘贴回应用后完成 OAuth 换 token；后端通过 `github.com/lfhy/xpan` 接入百度文件浏览、下载、上传、复制、移动、重命名与删除，并按百度限制把上传内容先写到 `/apps/网盘demo/` 再移动到最终目标路径；百度账号下暂不开放分享管理与本地挂载入口，避免走到现有 S3 专属链路。
