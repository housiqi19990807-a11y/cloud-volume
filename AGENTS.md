...
- `go/storage/sftp_backend.go` implements `MountPrefetchPolicy=false`: Finder root listing no longer launches speculative reads of up to eight child directories (for example `etc`, `proc`, and `dev`) over separate SSH connections. Its TCP dial uses `DialContext`, the SSH handshake inherits the request deadline, and cancellation closes the socket, so mount directory reads cannot silently exceed the mount request timeout during connect/handshake. Explicit directory navigation and the bounded active-directory poller remain unchanged.
