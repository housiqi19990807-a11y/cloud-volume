...
- `lib/widgets/file_manager_bucket_browser.dart` uses the existing `FileListTile` size column as a responsive “配额” column; `lib/widgets/file_manager_bucket_quota.dart` owns total resolution and list/grid label formatting. It prefers `CustomQuotaBytes` for the total, falls back to `BucketInfo.quotaBytes`, and shows `used / total` when `quotaKnown` is true; grid items use a compact same-unit form. It shows `--` when neither total exists. `lib/utils/transfer_format.dart` formats TB values as well as smaller units.
