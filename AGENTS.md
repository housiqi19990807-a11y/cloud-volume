...
- `lib/widgets/file_manager_bucket_browser.dart` uses the existing `FileListTile` size column as a responsive “已用 / 配额” column; `lib/widgets/file_manager_bucket_quota.dart` owns total resolution and list/grid formatting. Every bucket renders a capacity track in both views. It prefers `CustomQuotaBytes` for the total and falls back to `BucketInfo.quotaBytes`: `quotaKnown` fills the track from real usage, total-only backends such as generic S3 show a neutral empty track labeled `用量未知 · total`, and buckets without any total show `未设置额度` plus the same empty track. `FileListTile.sizeWidget` allows the richer capacity cell without changing other list rows. `lib/utils/transfer_format.dart` formats TB values as well as smaller units.
...
- `lib/services/remote_storage_gateway.dart` requires `getBucketQuota` so the second-stage request cannot be skipped by a failed runtime capability check; desktop implements it in `remote_storage_api_desktop_storage.dart` and Web forwards it through `remote_storage_api_web_transfers.dart` / `go/webapi/invoke.go` (`remote_storage_api_web.dart` stays as the thin HTTP/session shell). `file_manager_page.dart` renders initial buckets first, then `file_manager_page_quota.dart` schedules quota work in a post-frame callback, concurrently requests quota for every listed entry, logs Flutter/FFI failures through `AppLog.error`, and replaces only matching bucket IDs; unsupported S3 backends return unknown locally without a remote call. `lib/utils/app_log.dart` forces debug logging in Debug builds, while `go/logging/logging.go` always preserves error-level lines even when routine diagnostics are silent. `go/storage/baidu_pan_quota_test.go` verifies the SDK request carries the account token plus `checkfree=1` / `checkexpire=1` and maps `total` / `used`. `go/storage/webdav_quota_test.go` covers immediate list plus second-stage quota mapping and unsupported-property fallback; it is split from `webdav_backend_test.go` to keep both files below 500 lines. `test/widget_test.dart` verifies the initial unknown quota is later replaced in the rendered file manager. `test/bucket_quota_test.dart` covers provider JSON parsing plus custom/provider/unset display precedence.
