...
Relevant files and flow: `go/mount/bucket_access.go` owns the session-scoped `externalDelete` projector; `go/mount/bucket_access_reads.go` cancels pending writeback and calls it from `MarkExternalDelete`; `go/mount/backend_windows_cloud_files_cgo.go` installs/clears the projector and consumes provider-delete callback markers; `go/mount/cloud_files_external_delete_windows.go` validates the path stays below the sync root and removes the placeholder; `go/mount/cloud_files_watcher_state_windows.go` owns provider-delete and ordinary watcher state. Watcher lifecycle tests are split between `cloud_files_watcher_windows_test.go` and `cloud_files_watcher_lifecycle_windows_test.go` to keep both files below 500 lines. File-list delete is `delete_object` -> remote soft delete -> `NotifyExternalDelete` -> cancel writeback -> cache tombstone -> local placeholder removal. Explorer delete remains CFAPI delete completion -> `handleDelete` -> `deletePath` -> async remote delete.
