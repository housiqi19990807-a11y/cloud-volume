...
- `lib/services/app_exit_cleanup.dart` - Stores the bootstrapped desktop gateway and coalesces exit-time `cleanupMounts` calls. Cleanup failures are swallowed so an unavailable stale mount cannot trap the user in the app.
