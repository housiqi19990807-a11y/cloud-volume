...
Windows mounts can now choose between the Cloud Files shell (default) and a WinFsp-backed virtual file system that reports a real volume with a user-configured capacity to Explorer. WinFsp is optional: the default bridge build excludes the cgofuse dependency (no WinFsp header requirement), and the WinFsp engine compiles in only when the bridge is built with `-tags winfsp` and `third_party/winfsp/inc/fuse` headers are present.
...
- `scripts/run_windows.ps1` — sets `CPATH=third_party/winfsp/inc/fuse` and adds `-tags winfsp` to the bridge build when those headers exist.
...
- `third_party/winfsp/inc/{fuse,fuse3,winfsp}` — WinFsp 2.1 headers extracted from the MSI for local `-tags winfsp` builds without a system install.
- `go/mount/embedded/winfsp.msi` — WinFsp 2.1.25156 installer payload embedded via `go:embed` and reused by the installer.

#### Gotchas

- The `winfsp` build tag is required for the WinFsp engine. Without it (the default dev/release build), `newWindowsWinFspBackend` returns a clear error and the settings UI hides the WinFsp option, even when the driver is installed. `run_windows.ps1` adds the tag automatically when `third_party/winfsp/inc/fuse/fuse_common.h` is present.
- cgofuse's cgo variant (`host_cgo.go`) requires the WinFsp fuse headers via `CPATH` (or `-I/usr/local/include/winfsp`). Missing headers fail the bridge build with `fatal error: 'fuse_common.h' file not found`. The `winfsp` tag + stub files exist precisely to keep the default build free of that dependency.
