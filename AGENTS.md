...
**Download integrity (2026-07-06 fix):** 一键更新报「macOS 安装失败：挂载 DMG 失败：映像数据已损坏」。根因：`downloadInstaller` 收到 HTTP 响应后只按 body 流写盘，下载完成不做任何大小校验；部分加速镜像用 HTTP 200 返回截断内容或 HTML 错误页，被原样写入 `.dmg`，到 `hdiutil attach` 时才暴露为「映像数据已损坏」。修复：(1) `bridge/app_install_download.go` 新增 `verifyDownloadedSize(destPath, expectedSize)`，下载并显式 `f.Close()` flush 后 stat 落盘文件，与 GitHub asset `assetSize` 不一致时删除残留文件并报「下载文件大小不匹配……镜像可能返回了截断或错误内容」；`expectedSize <= 0` 时跳过校验（无法验证，留给安装阶段判错）。(2) `bridge/dispatch_app_install.go` `probeDownloadURL` 增加 `expectedSize` 参数，当镜像 HEAD 返回的 `Content-Length > 0` 且与 `assetSize` 不一致时直接报「镜像报称大小为 N 字节，与 GitHub Release 的 M 字节不一致」，在下载前就拒绝坏镜像。(3) `downloadInstaller` 写文件由 `defer f.Close()` 改为显式 `f.Close()`，保证 stat 前数据已落盘。测试 `bridge/app_install_download_test.go` 覆盖大小匹配、不匹配（且文件被删除）、无期望大小三种情形。
...
- `bridge/dispatch_app_install.go` — Go `install_app`: download (mirror/proxy), platform install (DMG/ZIP/exe/AppImage/tar), relaunch, `os.Exit(0)`; progress via `s3ops` transfer monitor. `probeDownloadURL` HEAD-probes wrapped mirror URL (with `expectedSize` Content-Length check) before download.
- `bridge/app_install_download.go` — Resumable installer download (`downloadInstaller`, HTTP Range), cache dir resolution, and `verifyDownloadedSize` post-download integrity check against GitHub asset size.
