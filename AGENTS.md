...
- `scripts/build_windows.bat` - Double-click launcher for release builds. It changes to the repo root and calls `scripts/run_windows.ps1 -Build`, producing the Windows bundle under `build/windows/x64/runner/Release/`. Forwards extra command-line arguments and pauses unless `CLOUD_VOLUME_NO_PAUSE=1`.
...
**Windows green ZIP update (2026-07-07):** `matchPlatformAsset` now prefers `yunjuan-windows-amd64.zip` for Windows and only falls back to `yunjuan-windows-amd64-installer.exe` when the ZIP is absent; the Go installer path supports both. `installWindowsZip` writes a temporary PowerShell updater under `%TEMP%\cloud-volume-updater`, starts it hidden, then the main app marks the update done and exits. The updater waits for the old PID to exit, expands the ZIP to a staging directory, finds `cloud-volume.exe` at the ZIP root or first child directory, copies the payload over the current install directory, launches the new `cloud-volume.exe`, and deletes its staging/script files. This makes green-package auto-update the default path, while the installer EXE remains available as fallback.
