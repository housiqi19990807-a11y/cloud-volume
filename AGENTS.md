...
- `go/storage/sftp_backend.go` / `sftp_backend_io.go` / `sftp_backend_test.go` - The SFTP backend currently establishes a fresh SSH/SFTP connection per operation. Tracked uploads start the queued transfer, report copied bytes through a context-aware reader, and finish success/failure in the shared monitor. The writeback success path then performs a separate `HeadObject`, so handshake latency and server connection limits still affect many-small-file throughput without changing the intended local-first foreground semantics.
- `go/s3/transfer_monitor.go` / `lib/state/transfer_task.dart` - Mount writeback first registers a `pending` task with `sync_wait`. Both S3 and SFTP upload code consume the task ID and advance/finalize the shared monitor, so a successful SFTP writeback leaves `sync_wait` immediately instead of waiting for the monitor's 10-minute pruning window.
...
- SFTP `UploadFile` and `UploadReader` consume non-empty transfer task IDs: they start the queued task, advance byte progress during `io.Copy`, honor cancellation through the reader context, and finish the task on success or failure. Keep the protocol-level regression test asserting a completed `14/14` snapshot so mount writeback cannot regress to a stale `sync_wait` row.
