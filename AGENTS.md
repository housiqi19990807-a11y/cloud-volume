...
- `scripts/build_windows.bat` - Double-click launcher for release builds. It changes to the repo root and calls `scripts/run_windows.ps1 -Build`, producing the Windows bundle under `build/windows/x64/runner/Release/`, then opens that output directory in Explorer on success. Forwards extra command-line arguments and pauses unless `CLOUD_VOLUME_NO_PAUSE=1`.
