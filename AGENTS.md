...
- `lib/widgets/file_sync_profile_editor.dart` — Dialog for creating or editing a single `SyncProfile`. **2-step wizard (2026-06-26):** Step 1 同步两端 (optional name, local dir via `FilePicker`, pick a bucket from the file-manager bucket list, remote prefix), Step 2 同步策略 (direction, conflict policy, interval, quiet period, exclude rules, enabled toggle). No manual account selection — the bucket list entry carries `profileName` so `accountProfile` is auto-bound. Name is optional (defaults to bucket name). Receives `List<FileManagerBucketEntry> buckets` instead of `List<ProfileInfo> profiles`.
