...
- `bridge/app_install_download.go` — Resumable installer download (`downloadInstaller`, HTTP Range), cache dir resolution, and post-download integrity checks: `verifyDownloadedSize` (byte count vs GitHub asset `size`) followed by `verifyDownloadedDigest` (SHA-256 vs GitHub asset `digest` `sha256:<hex>`); cache path also re-verifies digest before reuse.
