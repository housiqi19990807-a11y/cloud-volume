...
- `go/mount/webdav_quota.go` / `webdav_quota_test.go` / `bucket_access.go` - Root read handles expose RFC 4331 `quota-available-bytes` and `quota-used-bytes` through `webdav.DeadPropsHolder`, while exact-`O_RDWR` metadata handles still omit that interface. The mount keeps a full-config quota provider (so scoped WebDAV roots stay scoped), caches results for 30 seconds, prefers `BucketSettings.CustomQuotaBytes` for total capacity, retains provider used bytes, and clamps used to total. End-to-end PROPFIND tests cover provider capacity and custom-total precedence.
