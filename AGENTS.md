...
- `scripts/build_windows_installer.ps1` - Builds yunjuan-windows-amd64-installer.exe from the Flutter release bundle via Inno Setup 6 (ISCC.exe). Resolves ISCC from standard Program Files paths; uses git describe for the version label unless -Version is passed. Requires the release bundle (run scripts/build_windows.bat first).
- `scripts/build_windows_installer.bat` - Double-click launcher for installer packaging; calls uild_windows_installer.ps1 and opens Explorer on the output file.` - Double-click launcher for release builds. It changes to the repo root and calls `scripts/run_windows.ps1 -Build`, producing the Windows bundle under `build/windows/x64/runner/Release/` with a Git-derived version label, then opens that output directory in Explorer on success. Forwards extra command-line arguments (including `-Version x.y.z`) and pauses unless `CLOUD_VOLUME_NO_PAUSE=1`.
- `README.md` - Local development docs point new Windows machines to double-click `scripts/setup_windows_dev.bat` or run `powershell -ExecutionPolicy Bypass -File .\scripts\setup_windows_dev.ps1`, then use `scripts/run_windows_debug.bat`, `scripts/build_windows.bat
- `scripts/build_windows_installer.ps1` - Builds yunjuan-windows-amd64-installer.exe from the Flutter release bundle via Inno Setup 6 (ISCC.exe). Resolves ISCC from standard Program Files paths; uses git describe for the version label unless -Version is passed. Requires the release bundle (run scripts/build_windows.bat first).
- `scripts/build_windows_installer.bat` - Double-click launcher for installer packaging; calls uild_windows_installer.ps1 and opens Explorer on the output file.`, or `scripts/run_windows.ps1` for normal project run/build.
