...
- `go/mount/webdav_quota.go` / `webdav_quota_test.go` / `bucket_access.go` - Root read handles expose RFC 4331 `quota-available-bytes` and `quota-used-bytes` through `webdav.DeadPropsHolder`, while exact-`O_RDWR` metadata handles still omit that interface. `newBucketAccess` seeds the first response from `storage.CachedBucketQuota`; it prefers `BucketSettings.CustomQuotaBytes`, retains provider used bytes, and clamps used to total. There is no synchronous quota request in macOS mount startup. A cache miss stays non-blocking and starts the existing 30-second in-session background refresh, while the normal bucket-list-first UI path gives `webdavfs` capacity on its first PROPFIND. Tests cover seeded first-request quota, non-blocking unknown quota, background refresh, and custom-total precedence.
- `go/mount/macos_mount.go` / `macos_mount_test.go` / `macos_command.go` / `macos_command_test.go` / `system_mounts.go` - Default mounts use AppleScript and custom paths use `mount_webdav`, both under command timeouts. A timeout/non-zero/empty-output result is not automatically a failed mount: the recovery path polls the system WebDAV mount table for two seconds, matches either the requested path or the URL-derived decoded volume name (including macOS numeric suffixes), and returns that actual path when the OS already created the volume. `exec.Cmd.WaitDelay` also bounds inherited output pipes, so an `open` helper canceled after five seconds cannot leave the bridge waiting tens of seconds. During stale cleanup, a successful mount-table probe that says the volume is still active skips blocking `os.Stat` and proceeds directly to the next `diskutil` fallback.
...
- `go/storage/sftp_backend.go` implements `MountPrefetchPolicy=false`: Finder root listing no longer launches speculative reads of up to eight child directories (for example `etc`, `proc`, and `dev`) over separate SSH connections. Explicit directory navigation and the bounded active-directory poller remain unchanged.
