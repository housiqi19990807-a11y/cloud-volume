...
**Cross-client mutation diagnosis (verified 2026-08-14):** The remaining mutation and visibility reports have three separate boundaries. `cloud_files_watcher_windows.go` -> `bucket_access_writes.go` -> `overlay_bridge.go` stages a newly created directory locally and sends its remote `CreateDirectory` through the independent `dir_sync_queue.go`; `writeback_rename_queue.go` drains file uploads but does not order, rebase, or cancel that directory-create job. A directory rename can therefore finish remotely before the queued create for the old name, leaving both directory markers. The current rename regression only covers file uploads before/after a directory barrier, so this directory race is not pinned by a test. For ordinary file moves, `backend_windows_cloud_files_cgo.go` queues the remote move after Explorer has already changed the local projection. It passes no `reportAttempt` callback, so `writeback_rename_queue.go` only logs retry failures; the rename operation is not persisted in `writeback_store.go` and is dropped on shutdown/restart. A partial copy-then-delete failure can consequently look correct in Windows while the web listing still contains the source and destination. The exact remote call path is `bucketAccess.renamePath` -> `storage.Backend.MoveObject` -> either JWanFS `file-move` or S3 copy + captured-key hard delete (`go/s3/object_moves.go`). `remote_poller.go` separately expires directory activity after `remotePollWarmWindow` (3 minutes); Cloud Files records activity only when `OnFetchPlaceholders` runs, and P2P is opt-in/off by default, so an already-open but idle Windows directory will not discover Linux uploads after that window even when the web server is current. Evidence anchors: `go/mount/writeback_rename_queue_test.go` passes for the covered file-order case, while there are no tests for `dir_sync_queue` versus rename ordering, rename recovery across restart, or an expired-but-visible Cloud Files directory. The local `dist/release/yunjuan-windows-x64-installer.exe` predates commit `ad0ae7ce` (2026-08-10 build versus the 2026-08-11 fix), and the local `bridge.log` is zero bytes because Release logging remains Silent unless Debug is explicitly enabled.
