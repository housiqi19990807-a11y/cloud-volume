...
- `lib/services/remote_storage_gateway.dart` requires `getBucketQuota` so the second-stage request cannot be skipped by a failed runtime capability check; desktop implements it in `remote_storage_api_desktop_storage.dart` and Web forwards it through `remote_storage_api_web_transfers.dart` / `go/webapi/invoke.go` (`remote_storage_api_web.dart` stays as the thin HTTP/session shell). `file_manager_page.dart` owns a page-session quota cache, `file_manager_page_bucket_loading.dart` reapplies cached values before the bucket list renders, and `file_manager_page_quota.dart` schedules quota work in a post-frame callback. Successful quota results are cached by profile/bucket for 5 minutes: returning from a bucket reuses a fresh value without another bridge request; an expired value stays visible while the refresh runs. Cache entries are discarded when quota-relevant account settings (storage type, endpoint, access/refresh credentials, WebDAV credentials, or root prefix) change. Missing/stale entries refresh concurrently, Flutter/FFI failures are logged through `AppLog.error`, and only matching bucket IDs are replaced; unsupported S3 backends return unknown locally without a remote call. `lib/utils/app_log.dart` forces debug logging in Debug builds, while `go/logging/logging.go` always preserves error-level lines even when routine diagnostics are silent. `go/storage/baidu_pan_quota_test.go` verifies the SDK request carries the account token plus `checkfree=1` / `checkexpire=1` and maps `total` / `used`. `go/storage/webdav_quota_test.go` covers immediate list plus second-stage quota mapping and unsupported-property fallback; it is split from `webdav_backend_test.go` to keep both files below 500 lines. `test/widget_test.dart` verifies the initial unknown quota is later replaced and reused after entering/leaving a bucket. `test/bucket_quota_test.dart` covers provider JSON parsing plus custom/provider/unset display precedence.
...
- `go/mount/webdav_quota.go` / `webdav_quota_test.go` / `bucket_access.go` - Root read handles expose RFC 4331 `quota-available-bytes` and `quota-used-bytes` through `webdav.DeadPropsHolder`, while exact-`O_RDWR` metadata handles still omit that interface. The mount keeps a full-config quota provider (so scoped WebDAV roots stay scoped), prefers `BucketSettings.CustomQuotaBytes` for total capacity, retains provider used bytes, and clamps used to total. Provider refresh remains asynchronous and cached for 30 seconds, but macOS startup primes it for at most two seconds before starting the loopback server because `webdavfs` caches the first root PROPFIND capacity for the lifetime of the mount. Fast providers therefore populate `df` immediately; a blocked SFTP `StatVFS` still cannot recreate the old 20-second mount hang. Tests cover first-request provider quota, bounded prime timeout, non-blocking later PROPFIND, and custom-total precedence.
- `go/mount/macos_mount.go` / `macos_mount_test.go` / `system_mounts.go` - Default mounts use AppleScript and custom paths use `mount_webdav`, both under command timeouts. A timeout/non-zero/empty-output result is not automatically a failed mount: the recovery path polls the system WebDAV mount table for two seconds, matches either the requested path or the URL-derived decoded volume name (including macOS numeric suffixes), and returns that actual path when the OS already created the volume. This keeps the loopback server alive instead of leaving a dead mounted volume after an `osascript` timeout. During stale cleanup, a successful mount-table probe that says the volume is still active skips `os.Stat` (which can block for tens of seconds when the loopback server is gone) and proceeds directly to the next `diskutil` fallback.
- `go/mount/bucket_access_reads.go` / `bucket_cache.go` / `bucket_access_stat_cache_test.go` - `statPath` treats a fresh full directory listing as authoritative for both hits and misses, and treats an absent child of a mount-created local directory as locally missing. Finder probes every destination before `PUT`; these negative-cache paths prevent a many-small-file copy from performing one synchronous SFTP `HeadObject`/connection handshake per file. Expired or unknown directories still fall through to the provider so remote overwrites remain discoverable.
