...
- `go/mount/webdav_quota.go` / `webdav_quota_test.go` / `bucket_access.go` - Root read handles expose RFC 4331 `quota-available-bytes` and `quota-used-bytes` through `webdav.DeadPropsHolder`, while exact-`O_RDWR` metadata handles still omit that interface. The mount keeps a full-config quota provider (so scoped WebDAV roots stay scoped), prefers `BucketSettings.CustomQuotaBytes` for total capacity, retains provider used bytes, and clamps used to total. Provider refresh is asynchronous and cached for 30 seconds: an uncached root PROPFIND returns custom/stale/unknown quota immediately and starts one background lookup, so a slow SFTP `StatVFS` cannot block `webdavfs` mount negotiation. Tests hold the provider response open to prove the initial PROPFIND remains non-blocking, then verify the refreshed provider values and custom-total precedence.
