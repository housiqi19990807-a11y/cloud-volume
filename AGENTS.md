...
- `lib/widgets/file_sync_profile_editor.dart` — Dialog for creating or editing a single `SyncProfile`. **Redesigned (2026-06-26) as a 3-step wizard:** Step 1 基础信息 (name + account), Step 2 同步目标 (bucket, remote prefix, local dir via `FilePicker`), Step 3 同步策略 (direction, conflict policy, interval, quiet period, exclude rules, enabled toggle). Each step validates before advancing (`_next`/`_back`). Step indicator with dots + progress bar. Exposes `markDirty(VoidCallback)` so the steps part file can trigger rebuilds.
- `lib/widgets/file_sync_profile_editor_steps.dart` — Part file with top-level functions `stepBasicInfo`, `stepSyncTarget`, `stepSyncStrategy` that build each step's widget tree. Receives `_FileSyncProfileEditorState self` to access fields/controllers and calls `self.markDirty(...)` for setState.
