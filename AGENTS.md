...
- `lib/widgets/file_manager_bucket_browser.dart` uses the existing `FileListTile` size column as a responsive “已用 / 配额” column; `lib/widgets/file_manager_bucket_quota.dart` owns total resolution and list/grid formatting. It prefers `CustomQuotaBytes` for the total and falls back to `BucketInfo.quotaBytes`. Any known total renders a capacity track in both list and grid views: `quotaKnown` fills it from real usage and shows `used / total`, while total-only backends such as generic S3 show a neutral empty track labeled `用量未知 · total` instead of pretending usage is zero. A missing total renders `--`. `FileListTile.sizeWidget` allows the richer capacity cell without changing other list rows. `lib/utils/transfer_format.dart` formats TB values as well as smaller units.
