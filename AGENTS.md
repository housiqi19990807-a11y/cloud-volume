...
- `go/storage/webdav_quota.go` performs a depth-0 RFC 4331 PROPFIND against the mapped root, parses `DAV:quota-used-bytes` and `DAV:quota-available-bytes`, and reports total as used + available. Support is optional; HTTP/XML/missing-property failures are recorded through central `logging.Errorf` and `ListBuckets` still returns the mapped root without quota.
- `go/storage/baidu_pan_backend.go` calls the pinned xpan client's account-level `Client.Quota()` with `checkfree=1` and `checkexpire=1`, maps `total` / `used` into `BucketInfo`, and records failures through central `logging.Errorf` before degrading to an unannotated bucket. Token refresh and per-account proxy behavior continue through `withBaiduPanClient`; quota failure does not block the file manager.
- `go/storage/baidu_pan_quota_test.go` verifies the SDK request carries the account token plus `checkfree=1` / `checkexpire=1` and maps `total` / `used`. `go/storage/webdav_backend_test.go` covers successful WebDAV quota mapping and unsupported-property fallback. `test/bucket_quota_test.dart` covers provider JSON parsing plus custom/provider/unset display precedence.
