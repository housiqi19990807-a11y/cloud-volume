...
**Shutdown lifecycle finding (2026-07-15):** `go/mount/manager.go` has `CleanupMounts`, and the Windows backend `Stop` correctly calls provider `Disconnect` then `Deregister`, but the normal application exit paths do not invoke `cleanup_mounts`. The settings-page force-reset button is the only Flutter caller found. Confirmed close exits call `WindowControls.exitApp` -> native `ExitApplication` -> `Destroy`; the tray menu calls `ExitApplication` directly. Thus normal/tray exit can terminate without deregistering Cloud Files. Explicit unmount and next-start stale cleanup still deregister, but they are delayed recovery rather than exit-time cleanup.
