...
- Delete task progress is item-based (TotalItems/AdvanceTransferItems), not byte-based: the summary bar in `BatchTaskProgressDialog` prefers `totalItems > 0` over bytes. A trash-move reports 2N item steps for N objects (N copies + N source deletes) because both phases share one taskID; byte progress still updates for speed/size labels.
