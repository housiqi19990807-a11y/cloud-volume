...
- `lib/pages/file_manager_page_actions.dart` (`_runObjectAction`, delete branch) and `lib/pages/file_manager_page_selection.dart` (`_deleteSelectedObjects`) — confirm via `showDeleteObjectDialog` / `showDeleteObjectsDialog` (`lib/widgets/object_action_dialogs.dart`; text says 不可撤销 even though trash is on by default), then `_queueObjectDeletes`.
- `lib/pages/file_manager_page_object_deletes.dart` — `_queueObjectDeletes` starts `TransferKind.delete` tasks and runs `_runDeleteTask` -> `api.deleteObject(config, bucket, key, isDir, taskId)`; failures are collected and surfaced via `_showPageMessage(title: '删除失败', ...)` (raw `error.toString()`, which keeps the `RemoteStorageBridgeException:` prefix).
...
- `go/s3/object_mutations.go` — soft `DeleteObjectContext(WithTask)` delegates to `MoveObjectToTrashContext` (`go/s3/trash_ops.go:29`); hard `DeleteObjectHardContext` lists `mutationKeys` then per-key `client.DeleteObject` (sequential, no DeleteObjects batch API).
