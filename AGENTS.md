...
- `lib/widgets/file_manager_bucket_browser.dart` uses the existing `FileListTile` size column as a responsive “已用 / 配额” column; `lib/widgets/file_manager_bucket_quota.dart` owns total resolution and list/grid formatting. It prefers `CustomQuotaBytes` for the total, falls back to `BucketInfo.quotaBytes`, and shows a full `used / total` value plus a thin usage bar and tooltip when `quotaKnown` is true. Unknown usage with a custom total renders `-- / total`, grid items use compact `已用 n/n unit` text, and a missing total renders `--`. `FileListTile.sizeWidget` allows the richer capacity cell without changing other list rows. `lib/utils/transfer_format.dart` formats TB values as well as smaller units.
