...
**Windows green ZIP update (2026-07-07):** Windows update asset selection already falls back from `yunjuan-windows-amd64-installer.exe` to `yunjuan-windows-amd64.zip`; the Go installer path now supports that ZIP fallback. `installWindowsZip` writes a temporary PowerShell updater under `%TEMP%\cloud-volume-updater`, starts it hidden, then the main app marks the update done and exits. The updater waits for the old PID to exit, expands the ZIP to a staging directory, finds `cloud-volume.exe` at the ZIP root or first child directory, copies the payload over the current install directory, launches the new `cloud-volume.exe`, and deletes its staging/script files. This enables green-package auto-update without Inno Setup, while the installer EXE remains preferred when present.
