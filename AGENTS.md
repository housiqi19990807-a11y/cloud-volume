...
**Windows green ZIP update (2026-07-08):** `matchPlatformAsset` now prefers `yunjuan-windows-amd64.zip` for Windows and only falls back to `yunjuan-windows-amd64-installer.exe` when the ZIP is absent; the Go installer path supports both. `installWindowsZip` extracts `cloud-volume-updater.exe` from the downloaded zip to a temp directory (old versions don't need to pre-install it) and launches it with `-zip`, `-install-dir`, `-pid`, `-exe-name`. The updater (`cmd/cloud-volume-updater/main.go`) shows a custom-painted Win32 progress window (not a MessageBox) matching the project style: light background, Microsoft YaHei UI font, the 云卷 wordmark, a step label, and a segmented progress bar (DWM rounded corners on Windows 11), waits for the old PID to exit, polls for file lock release, extracts the zip to a staging dir, copies the payload over the install dir (skipping its own exe), then relaunches `cloud-volume.exe` and waits for the new process to actually start before exiting (avoiding restart races). Every step is logged to %TEMP%\cloud-volume-updater-<pid>.log. This replaces the previous PowerShell-script approach which was fragile (hidden window, execution-policy issues, silent failures). The updater EXE is built by `run_windows.ps1 -Build` and `build_desktop_packages.sh build_windows`, and ships inside the release zip so it can be extracted on demand during updates.
...
- `cmd/cloud-volume-updater/updater_window_windows.go` — Custom-painted Win32 progress window (RegisterClassEx + CreateWindowEx + WM_PAINT): renders the Yunjuan wordmark, step label, and a segmented progress bar; requests DWM rounded corners on Windows 11; runs update on a goroutine while the main thread pumps messages.
...
- `lib/pages/file_manager_page_actions.dart` (`_runObjectAction`, delete branch :358-364) and `lib/pages/file_manager_page_selection.dart` (`_deleteSelectedObjects` :164-179) — confirm via `showDeleteObjectDialog` / `showDeleteObjectsDialog` (`lib/widgets/object_action_dialogs.dart:196` and `:241`; both return `Future<bool>` (null dismiss → false), `ShadDialog` width 380 + outline-cancel/destructive-delete, copy says "此操作不可撤销" even though trash is on by default; neither dialog takes a trash parameter), then `_queueObjectDeletes` + `_showDeleteProgressDialogForTasks` (`lib/pages/file_manager_page_upload_feedback.dart:16` → `BatchTaskProgressDialog` with `BatchTaskProgressMode.delete`).
- `lib/pages/file_manager_page_object_deletes.dart` — `_queueObjectDeletes` (:8) starts `TransferKind.delete` tasks (`localPath: ''`) and runs `_runDeleteTask` (:61) -> `api.deleteObject(config, bucket, key, isDir, taskId)` + cache evict + `markTaskDone/markTaskFailed`; failures are collected and surfaced via `_showPageMessage(title: '删除失败', ...)` (raw `error.toString()`, which keeps the `RemoteStorageBridgeException:` prefix).
- `lib/services/remote_storage_api_desktop_storage.dart:69` / `lib/services/remote_storage_api_web_objects.dart:61` — gateway `deleteObject` -> bridge op `delete_object` with `{config, bucket, key, isDirectory, taskId}`.
- `bridge/dispatch.go:66` (`delete_object` case) and `:293` (handler) -> `storageops.ForConfig(cfg).DeleteObject(ctx, bucket, key, isDirectory, taskID)`; on success calls `bucketmount.NotifyExternalDelete`. Web path: `go/webapi/invoke.go:222`. Trash ops: `bridge/dispatch.go:68-77` + `bridge/dispatch_trash.go` (`list_trash`, `list_trash_page` in `bridge/dispatch_paging.go`, `restore_trash_item`, `delete_trash_item`, `clear_trash`).
- `go/storage/s3_backend.go:66` — `DeleteObject` routes by per-bucket trash flag: trash disabled -> `s3ops.DeleteObjectHardContextWithTask`; trash enabled -> `s3ops.DeleteObjectContextWithTask` (soft delete). `go/config/config.go:397` `BucketSettingsFor` defaults `TrashEnabled=true` for `StorageTypeS3`, overridable per bucket (`bucketSettings[bucket].trashEnabled`, `trashDirectory`); Dart mirror `lib/models/remote_storage_config.dart:418` + `lib/models/bucket_settings.dart`.
- `go/s3/object_mutations.go` — soft `DeleteObjectContext(WithTask)` (:30, `startTransfer(taskID,"delete",…,0,cancel)` :46) delegates to `MoveObjectToTrashContext` (`go/s3/trash_ops.go:29`); hard `DeleteObjectHardContext` (:62) lists `mutationKeys` then per-key `client.DeleteObject` (sequential, no DeleteObjects batch API). `DeleteObjectHardContextWithTask` (:81) also `startTransfer(…,0,cancel)` :97 and performs NO `advanceTransfer` calls — hard-delete task progress stays indeterminate.
- `go/s3/trash_ops.go` + `go/s3/trash_helpers.go` + `go/s3/trash_index.go` — `MoveObjectToTrashContext`: trims key, skips trash keys, `mutationKeys` (count), generates UUID, target = `<trashDir>/objects/<uuid>/<originalKey>` (trashDir default `.trash`, config `trashDirectoryName`), calls `MoveObjectContext` (copy + delete-source), then `buildTrashMetadata` + `persistTrashMetadata` (index objects under `.trash/index/`; legacy `.trash/entries/<id>.trashinfo.json` fallback). Retention purge: `go/s3/trash_purge_scheduler.go` (10 min cooldown, `trashRetentionDays`, default 30 / -1 disables).
- `go/s3/object_moves.go:70` `MoveObjectContextWithTask` — `buildObjectTransferPlan` (`object_transfer_plan.go`: `mutationEntries` ListObjectsV2 paginator under the prefix) -> `executeObjectCopyPlan` (`object_transfer_run.go:51`: per-entry `client.CopyObject`, directory placeholder keys become PutObject markers) -> `DeleteObjectHardContext` on the source. This copy phase is the CopyObject that appears in delete error messages. Soft-delete progress: `beginObjectTransferTask` (`object_transfer_run.go:16`, `startTransfer(…,plan.totalBytes,cancel)` :32) + per-entry `task.advance(entry)` -> `advanceTransfer(t.id, *entry.Size)` (:44-49), so soft deletes report real byte progress.
- Flutter trash flag: `lib/models/remote_storage_config.dart` `bucketSettingsFor` (:416, `defaultTrashEnabled = storageType == StorageType.s3` :418) / `bucketTrashEnabled` (:429); `lib/models/bucket_settings.dart` `BucketSettings.isTrashEnabled` (:48). File manager: `lib/pages/file_manager_page_bucket_policy.dart` `_bucketTrashEnabled` (:14) + `_activeBucketTrashEnabled` (:20) — already available at both delete call sites. Edit UI: `lib/widgets/bucket_settings_dialog.dart` (ShadSwitch inside ShadDialog via StatefulBuilder :24-63) — canonical switch-in-dialog pattern; `mount_bucket_dialog.dart:87` is the StatefulWidget variant.
- Hard-delete reachability: storage interface `go/storage/types.go:36` + `s3Backend.DeleteObjectHard` (`go/storage/s3_backend.go:82`) and mount delete queue (`go/mount/delete_queue.go:170`) use it, but there is NO `delete_object_hard` bridge/webapi op — file manager cannot trigger a forced hard delete today. `delete_trash_item` (`bridge/dispatch_trash.go:54`, Dart `remote_storage_api_desktop_storage.dart:156` / `remote_storage_api_web_objects.dart:175`) hard-deletes by `trashId` (`go/s3/trash_ops.go:145`) and is only for trash items.
...
- `go/mount/winfsp_fs_windows.go` + `go/mount/winfsp_fs_helpers_windows.go` — cgofuse `FileSystemInterface` over `bucketAccess` (`//go:build windows && cgo`; Getattr/Readdir/Open/Create/Read/Write/Truncate/Flush/Release/Mkdir/Unlink/Rmdir/Rename/Statfs). Reads/writes reuse the cache + writeback queue. `Statfs` reports `windowsWinFspCapacityGB` as total/free blocks so Explorer shows the custom capacity. Helper file holds Stat/error mapping to keep both files under 500 lines.
...
- `go/mount/winfsp_backend_windows_test.go` — unit tests for `hasWinFspMountSuffix`/`isWindowsDriveMount` (no driver needed).
...
- WinFsp reports capacity from `Statfs.Blocks * Frsize`; `winFspBlockBytes` is fixed at 4096 and `Bfree`/`Bavail` mirror total blocks because the backend is an unbounded object store, not a real disk.
