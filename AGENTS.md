...
- `lib/widgets/file_sync_profile_editor.dart` — Dialog for creating or editing a single `SyncProfile`. **2-step wizard (2026-06-26):** Step 1 同步两端 (optional name, local dir via `FilePicker`, remote dir via `RemoteDirectoryPickerDialog`), Step 2 同步策略 (direction, conflict policy, interval, quiet period, exclude rules, enabled toggle). The picker returns bucket + prefix + profileName + config, so `accountProfile` is auto-bound. Name is optional (defaults to bucket name). Receives `api` + `List<FileManagerBucketEntry> buckets`.
