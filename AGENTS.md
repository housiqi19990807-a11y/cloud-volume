...
- `lib/services/remote_storage_gateway.dart` defines optional `BucketQuotaQuery`; desktop implements it in `remote_storage_api_desktop_storage.dart` and Web forwards it through `remote_storage_api_web_transfers.dart` / `go/webapi/invoke.go` (`remote_storage_api_web.dart` stays as the thin HTTP/session shell). `file_manager_page.dart` renders initial buckets first, then `file_manager_page_quota.dart` concurrently refreshes WebDAV/Baidu entries and replaces only matching bucket IDs. `go/storage/baidu_pan_quota_test.go` verifies the SDK request carries the account token plus `checkfree=1` / `checkexpire=1` and maps `total` / `used`. `go/storage/webdav_quota_test.go` covers immediate list plus second-stage quota mapping and unsupported-property fallback; it is split from `webdav_backend_test.go` to keep both files below 500 lines. `test/widget_test.dart` verifies the initial unknown quota is later replaced in the rendered file manager. `test/bucket_quota_test.dart` covers provider JSON parsing plus custom/provider/unset display precedence.
