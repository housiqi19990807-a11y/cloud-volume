...
**Mounted-exit warning:** Before showing or bypassing the close dialog, `DesktopWindowControls` calls `AppExitCleanup.activeMountCount`. Any live mount forces a dialog explaining that Exit unmounts active roots and that "后台运行" preserves them. On Windows that action hides to tray; on Linux it minimizes while keeping the process and mounts alive.
