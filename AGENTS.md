...
- `go/s3/transfer_monitor.go` / `lib/state/transfer_task.dart` - Mount writeback first registers a `pending` task with `sync_wait`. S3 upload code consumes the task ID and advances/finalizes the shared monitor, but SFTP `UploadFile` currently ignores its task ID. A successful SFTP writeback can therefore delete its persisted queue record while the UI snapshot remains pending until `ListTransferSnapshots` prunes non-running tasks after 10 minutes. For SFTP, a visible ten-minute `sync_wait` is not reliable proof that bytes are still pending.
...
When diagnosing an SFTP row stuck in `sync_wait`, inspect both `<runtime>/mounts/<bucket>/writeback/queue-*.json` and the remote object. An empty persisted map means the writeback layer no longer has recoverable pending work even if the in-memory transfer snapshot is still visible. The fix must explicitly start/finish the shared transfer task around provider backends that do not own monitor reporting; pruning the stale snapshot sooner would only hide the lifecycle bug.
...
- SFTP `UploadFile` currently ignores the transfer task ID. Mount writeback queues the task as `sync_wait`, but unlike S3 nothing transitions or finishes that monitor entry after the SFTP copy succeeds; the persisted writeback record and remote object may already be complete while the UI row remains pending for the monitor's 10-minute retention window.
