// 全局回收站页：跨所有存储桶聚合软删除项目，并提供恢复与彻底删除操作。
...
class _GlobalTrashPageState extends State<GlobalTrashPage> {
  ...
  Future<void> _runBusy(
    _GlobalTrashEntry entry,
    Future<void> Function() action,
  ) async {
    setState(() => _busyEntries.add(entry.id));
    try {
      await action();
    } catch (error) {
      if (!mounted) {
      ...
      if (mounted) {
        setState(() => _busyEntries.remove(entry.id));
      }
    }
  }
  ...
  Widget _buildBody(ShadThemeData theme) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) => _buildEntryCard(theme, _entries[index]),
    );
  }
