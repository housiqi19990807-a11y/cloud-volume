// 更新检查区：把 GitHub Release 查询、状态提示和下载页跳转封装在关于页里。
...
class _SettingsUpdateSectionState extends State<SettingsUpdateSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          '检查 GitHub Release 是否有可用的新版本，并跳转到下载页获取安装包。',
          style: TextStyle(
            fontSize: 12,
            height: 1.6,
            ...
            ShadButton(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(_checking ? '检测中...' : '检测更新'),
                ],
              ),
            ),
            ShadButton.outline(
              onPressed: _openingDownloadPage ? null : _openDownloadPage,
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(LucideIcons.externalLink, size: 15),
                  const SizedBox(width: 8),
                  Text(_openingDownloadPage ? '打开中...' : '前往 GitHub 下载'),
                ],
              ),
            ),
          ],
  ...
  String get _statusSubtitle {
    final result = _result;
    if (_checking) {
      return '正在读取最新 Release 信息。';
    }
    if (result == null) {
      return '点击检测更新后会比较本地版本和 GitHub 最新发布版本。';
    }
    if (!result.comparable) {
...
class _UpdateStatusBlock extends StatelessWidget {
  const _UpdateStatusBlock({
    required this.theme,
    required this.title,
