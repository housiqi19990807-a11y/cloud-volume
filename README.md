...
- 文件预览：桌面端点击文件会先打开应用内预览弹框，图片可直接缩放查看；PDF、视频、Word 及暂不支持的格式会提示下载查看。弹框里的“外部应用打开 / 另存为 / 下载”会复用同一个拟态进度区域，外部打开在下载完成后自动交给系统应用并关闭弹框，下载和另存为完成后保留完成状态供确认；当被预览的文件在远端已被删除（404/NoSuchKey）时，弹框只保留关闭按钮，不再显示下载相关动作，关闭后会自动刷新当前目录让已删除的条目消失；文件列表右键和多选下载也会弹出统一下载进度框，并可切到后台继续执行；右键下载会先显示进度弹框，再继续选择本地保存路径；前台进度弹框中的任务也会同步点亮侧边栏“对象传输”；目录展开失败也会在任务行内展示错误，扫描完成后会显示总文件数和总字节数；百度网盘下载和文件信息读取会用带退避重试的路径查找，并复用列表阶段缓存的 fsid 获取下载地址，避免 SDK 内部频控被误报为 `file not found` 以及每个文件下载前重复 list；下载到本地路径时会优先复制已有文件内容缓存，目录递归下载中的已缓存子文件也不会重新请求远端下载，已浏览过的目录层级会复用页面对象列表缓存。；本地拖拽或粘贴上传的文件会在上传成功后把本地副本登记进预览缓存，双击打开刚上传的文件直接命中缓存秒开，不再重复下载，缓存写入失败也不影响上传成功。
...
- 文件同步：可指定一个本地目录定期同步到远端桶目录，支持仅上传、仅下载和双向同步三种方向；双向同步通过本地索引加三方比对识别新增、修改、删除与重命名，冲突时按较新、本地优先、远端优先或跳过策略处理；正在写入的热数据会等到静默指定秒数后才纳入同步，避免频繁操作远端；同步配置（新增、编辑、删除、启停）在独立的文件同步页面统一管理，同步产生的任务复用「传输」任务队列；文件同步页在每条配置卡片内展示当前最新进行中的同步项摘要。
- 桌面体验：托盘图标、透明标题栏、统一中文字体、面向桌面鼠标操作的上下文菜单和固定表头列表。
- macOS 宿主壳：关闭主窗口时会先确认“退出云卷 / 隐藏到托盘 / 取消”，隐藏到托盘会保留后台菜单栏入口，只有确认退出才结束应用。
- Windows 宿主壳：使用与 macOS 一致的 `云卷` 品牌图标，移除系统标题栏，在应用内右上角提供自定义最小化 / 最大化 / 关闭按钮，默认按主屏工作区居中打开窗口，并常驻系统托盘以支持隐藏和恢复主窗口；隐藏到托盘后会记住最大化状态，再次点击托盘或重新启动应用都会正确把主窗口带回屏幕，不再出现只剩托盘无法唤起主界面的情况。
...
- 设置页“通用设置”底部提供“账号重置”入口，确认后会清空所有已保存账号（含旧版 `~/.remote-storage` 与旧 Windows 安装目录下的遗留配置）并回到首次启动配置页；已挂载桶与缓存目录不受影响。
