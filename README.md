...
- 多账号与多后端：统一管理 S3 兼容对象存储、WebDAV 和百度网盘账号；首次启动、账号管理和文件管理都围绕同一套 profile 流程展开。
...
- 账号管理页直接展示所有账号，并保留和桶列表一致的卡片 / 表格视图切换；新增和编辑账号在桌面端打开独立子窗口（`desktop_multi_window`），Web 端回退到拟态框。子窗口里第一步选择存储类型，目前支持 S3 对象存储、WebDAV 和百度网盘，上游类型不再作为页面分组。账号添加后会直接共存，不需要额外“连接”或切换当前账号。
...
- Windows 一键更新优先使用 Inno Setup `installer.exe`；如果 Release 只有绿色版 `yunjuan-windows-amd64.zip`，也会下载 zip，启动临时 updater 脚本等待当前进程退出，解压覆盖当前应用目录并重新启动 `cloud-volume.exe`。
