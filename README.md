...
- 文件管理：桶列表、目录浏览、列表/网格视图、右键操作、搜索、多选、批量下载/删除；空白区和文件项右键都支持新建目录、上传文件，多选右键会直接提供批量下载、复制、移动和删除；新增或初始化账号时可配置映射到文件管理页的桶名称，桶列表支持直接打开“桶设置”并为当前桶单独配置只读、是否启用回收站、以及桶级回收站目录覆盖；WebDAV 目录会按当前层级探测只读权限并限制写入入口，根目录只读但子目录可写时会以当前子目录权限为准；当服务器 ACL 缺失时，会结合当前目录的 `OPTIONS Allow` 做保守判断，只在明确只读方法集时拦截写入，避免把带 `LOCK/COPY/MOVE/PROPPATCH` 但未列出 `PUT` 的目录误判成只读。桌面端还支持把本地文件或文件夹拖入文件列表上传、复制本地文件或文件夹后在列表中粘贴上传、复制远端文件后粘贴到 Finder / Explorer 等本地目录；文件夹上传由 Go bridge 在后台扫描并更新传输进度，前端会先弹出统一的拟态进度框，显示已发现/已上传文件数量，并可直接取消或切到后台继续上传。文件列表/网格里也可以直接用鼠标拖出选框批量选择对象。
