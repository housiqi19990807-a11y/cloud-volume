...
文件管理首页聚合展示所有账号下的 bucket / 根目录，并在列表中标出来源账号、存储类型、回收站和挂载状态。
...
- 文件管理首页会聚合展示所有已保存账号下的 bucket / 根目录，并在“来源”列明确标出账号名和存储类型；列表视图可拖拽调整桶顺序（顺序会持久保存），方便直接并行管理多个上游。
...
The Windows Cloud Files mount dialog can optionally assign an available drive letter. The app maps the letter to the existing `~/Cloud Volume/<bucket>` sync root after Cloud Files startup, reports the selected letter in mount status, opens that drive from the mounted-bucket action, and removes the mapping during unmount, remount cleanup, or normal app-exit cleanup. Drive letters are selected automatically from `Z:` down through `D:` and are session mappings rather than separate filesystem volumes.
