...
- 文件同步：可指定一个本地目录定期同步到远端桶目录，支持仅上传、仅下载和双向同步三种方向；双向同步通过本地索引加三方比对识别新增、修改、删除与重命名，冲突时按较新、本地优先、远端优先或跳过策略处理；正在写入的热数据会等到静默指定秒数后才纳入同步，避免频繁操作远端；同步配置（新增、编辑、删除、启停）在独立的文件同步页面统一管理，同步产生的上传、下载、删除、重命名任务复用任务队列并在该页面展示进度。
