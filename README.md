...
首次启动会引导选择存储类型，目前支持 S3 兼容对象存储、WebDAV 和百度网盘。

![首次启动选择账号类型](docs/screenshots/init1.png)

S3 账号支持 endpoint、access key、secret key、region、path-style URL 和 bucket 映射名称等配置；WebDAV 和百度网盘走各自的授权流程。
...
文件管理首页聚合展示所有账号下的 bucket / 根目录，并在列表中标出来源账号、自定义配额、回收站和挂载状态。
...
- 桌面挂载：macOS 通过系统 WebDAV 卷挂载，Linux 通过 FUSE 挂载，Windows 支持 WebDAV、Cloud Files 与可选的 WinFsp 虚拟文件系统卷；支持只读/读写模式、本地优先缓存、overlay、异步写回和写回状态提示。桶级自定义配额会作为 Linux FUSE / Windows WinFsp 的文件系统总容量返回；WinFsp 未设置桶配额时回退到 Windows 高级设置中的全局虚拟容量。驱动缺失时支持应用内一键静默安装（内嵌 MSI）。
...
- 回收站与分享：提供应用级软删除、全局/桶级回收站、恢复、彻底删除、清空回收站，以及预签名分享链接的创建、续期、复制和删除。删除确认拟态框可在软删除时切换「永久删除」直接绕过回收站；目录删除按对象数实时显示确定进度条。移动/重命名的源清理使用枚举时固定的 key 列表，保证旧位置对象全部移除。软删除/移动/重命名的逐对象服务端复制与源删除自带容错重试，单个对象的临时性网关错误（502/超时等）不会直接中止整个目录操作。
...
- 列表页头部（任务队列、分享管理、回收站等）在批量选中后右侧操作按钮较多时，会自动按可用宽度把次要操作（移除记录、清空选择、清空已完成等）收进“…”更多操作下拉菜单，避免挤压标题导致副标题错位断行；空间充足时仍平铺全部按钮。
- 账号管理页直接展示所有账号，并保留和桶列表一致的卡片 / 表格视图切换；列表视图可拖拽调整账号顺序（顺序会持久保存）；新增和编辑账号默认打开应用内拟态框（`showAppModal`）；仅在 Debug 且开启 `USE_MODAL_SUB_WINDOWS` 时才使用独立 OS 子窗口。表单采用两步式引导——第一步选择接入协议（S3 对象存储 / WebDAV / 百度网盘），第二步填写对应的连接信息和代理配置；编辑模式跳过协议选择，直接进入连接信息。账号添加后会直接共存，不需要额外”连接”或切换当前账号。
- 文件管理首页会聚合展示所有已保存账号下的 bucket / 根目录，并在“来源”列明确标出账号名，在“配额”列展示桶设置中的自定义容量；列表视图可拖拽调整桶顺序（顺序会持久保存），方便直接并行管理多个上游。
...
- 设置页里的回收站目录仍是全局默认值，并提供“自动清理”显式开关与保留天数；每个桶还可以在文件管理首页的“桶设置”里单独覆盖回收站目录、直接关闭回收站，或填写自定义配额（GB）。自定义配额会显示在桶列表，并在重新挂载后同步到 Linux FUSE / Windows WinFsp 的文件系统容量统计；它不会限制上传，0 或留空表示未设置。Cloud Files 与 WebDAV 挂载的容量由宿主文件系统或 WebDAV 客户端决定。
...
- 如果配置缺失或不完整，会先进入初始化配置页；初始化流程会先选择账号类型，目前支持 S3 对象存储、WebDAV 和百度网盘，再进入对应账号表单。第一步仍是左右分栏（左侧品牌宣传 + 右侧协议选择）；进入第二步连接表单后会隐藏左侧宣传，改为全屏表单（宽屏下 S3 / WebDAV 字段两列排布），减少单页滚动。步骤切换时左侧品牌面板会平滑收起、右侧内容淡入，不再硬切或额外转圈。连接页左上角可点「返回」回到类型选择。
