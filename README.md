...
- 主界面围绕“文件管理、云存储管理、任务队列、回收站、分享管理、系统设置”六类核心页面展开。
- 云存储管理页采用“上游类型总览 / 类型内账号列表”两级页面：OSCA、GFS、MinIO、其他 S3 兼容对象存储先以系统卡片展示，再进入复用固定表头列表的账号管理页；可连接具体账号、退出账号，也可在不打开内部下拉浮层的固定弹窗里新增上游账号。
...
- 后续可在“云存储管理”里同时维护多个上游账号；先进入对应上游类型，再管理该类型下的独立 profile，新增账号不会覆盖当前正在连接的账号。
