...
- 多账号与多后端：统一管理 S3 兼容对象存储、WebDAV 和百度网盘账号；首次启动、账号管理和文件管理都围绕同一套 profile 流程展开。每个账号可单独选择跟随全局代理、跟随系统、直连或自定义 HTTP/SOCKS5 代理，并可用统一的桶 allowlist、显示名称与入口子目录控制文件管理视图。
