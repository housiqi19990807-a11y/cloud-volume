...
- 账号管理页直接展示所有账号，并保留和桶列表一致的卡片 / 表格视图切换；新增和编辑账号默认打开应用内拟态框（`showAppModal`）；仅在 Debug 且开启 `USE_MODAL_SUB_WINDOWS` 时才使用独立 OS 子窗口。表单采用两步式引导——第一步选择接入协议（S3 对象存储 / WebDAV / 百度网盘），第二步填写对应的连接信息和代理配置；编辑模式跳过协议选择，直接进入连接信息。账号添加后会直接共存，不需要额外”连接”或切换当前账号。
