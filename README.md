...
- 百度网盘 OpenAPI：账号管理和首次初始化现在支持“百度网盘”类型；桌面端会按百度要求使用 `oob` 模式打开浏览器授权页，用户把网页显示的授权码粘贴回应用后再换取 access token / refresh token 并保存到 profile。百度网盘上传会严格先落到 `/apps/网盘demo/`，再自动移动到用户最终选择的目标路径；当前先覆盖账号接入、目录浏览、下载、上传、复制、移动、重命名和删除，分享管理与本地挂载入口会在百度账号下自动隐藏。
