...
- 通用设置顶部提供“检测更新”、“一键更新”和“GitHub 下载”。“检测更新”会读取 GitHub 最新 Release 信息（API 直连，单次最长约 30 秒并带有限重试）；发现新版本后桌面端可直接点击“一键更新”在应用内自动下载安装包并替换旧版本，安装完成后自动重启，无需手动卸载重装或执行命令行命令。Web 端仍跳转 GitHub 下载页。一键更新下载安装包完成后会在本地校验文件大小与 GitHub Release 一致，不一致（多为加速镜像返回截断内容或错误页）时给出明确提示并清理残留文件，避免用坏包安装导致后续 `hdiutil`/解压报“映像数据已损坏”。
