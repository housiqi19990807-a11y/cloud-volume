...
文件管理首页聚合展示所有账号下的 bucket / 根目录，并在列表中标出来源账号、配额、回收站和挂载状态。WebDAV 与百度网盘会读取服务端真实配额，用户设置的自定义配额优先显示。
...
- 文件管理首页会聚合展示所有已保存账号下的 bucket / 根目录，并在“来源”列明确标出账号名；“配额”列优先展示桶设置中的自定义容量，未设置时读取 WebDAV RFC 4331 或百度网盘账号的真实总容量，不支持配额查询的后端显示 `--`。列表视图可拖拽调整桶顺序（顺序会持久保存），方便直接并行管理多个上游。
