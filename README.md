...
- 文件管理：桶列表、目录浏览、列表/网格视图、右键操作、搜索、多选、批量下载/删除；新增或初始化账号时可配置映射到文件管理页的桶名称，桶列表支持直接打开“桶设置”并为当前桶单独配置只读、是否启用回收站、以及桶级回收站目录覆盖；WebDAV 目录会按当前层级探测只读权限并限制写入入口，根目录只读但子目录可写时会以当前子目录权限为准；当服务器 ACL 缺失时，会结合当前目录的 `OPTIONS Allow` 做保守判断，只在明确只读方法集时拦截写入，避免把带 `LOCK/COPY/MOVE/PROPPATCH` 但未列出 `PUT` 的目录误判成只读。桌面端还支持把本地文件拖入文件列表上传、复制本地文件后在列表中粘贴上传，以及复制远端文件后粘贴到 Finder / Explorer 等本地目录；开始上传时会先弹出统一的拟态进度框，可直接切到后台继续上传。
- 百度网盘 OpenAPI：账号管理和首次初始化现在支持“百度网盘”类型；桌面端会监听本地 OAuth 回调端口、自动拉起浏览器完成授权，并把 access token / refresh token 保存到 profile。百度网盘上传会严格先落到 `/apps/网盘demo/`，再自动移动到用户最终选择的目标路径；当前先覆盖账号接入、目录浏览、下载、上传、复制、移动、重命名和删除，分享管理与本地挂载入口会在百度账号下自动隐藏。
- 文件预览：桌面端点击图片会打开独立预览子窗口并支持缩放查看；PDF、视频、Word 及暂不支持的格式仍会提示下载查看，并提供取消、另存为、下载操作。
...
- 任务队列：统一展示上传、下载、复制、移动、删除、挂载写回，以及 WebDAV 挂载触发的懒加载读取；挂载读取会聚合到单文件任务并显示当前 `bytes=start-end` range，支持筛选、多选、批量开始/取消，以及持久化恢复。
...
- 账号管理页直接展示所有账号，并保留和桶列表一致的卡片 / 表格视图切换；新增和编辑账号时第一步选择存储类型，目前支持 S3 对象存储、WebDAV 和百度网盘，上游类型不再作为页面分组。
...
- 设置页里的回收站目录仍是全局默认值；每个桶还可以在文件管理首页的“桶设置”里单独覆盖回收站目录或直接关闭回收站。
...
- S3 对象存储初始化需要填写 `endpoint/access_key_id/secret_access_key`，高级设置里可调整 `region` 与 path-style URL；WebDAV 初始化需要填写服务地址、用户名和密码；百度网盘初始化会通过桌面端本地 OAuth 回调完成授权，若 3 分钟内没有收到浏览器回调则会提示认证失败。
