...
- 回收站与分享：提供应用级软删除、全局/桶级回收站、恢复、彻底删除、清空回收站，以及预签名分享链接的创建、续期、复制和删除。删除确认拟态框可在软删除时切换「永久删除」直接绕过回收站；目录删除按对象数实时显示确定进度条。软删除/移动/重命名的逐对象服务端复制与源删除自带容错重试，单个对象的临时性网关错误（502/超时等）不会直接中止整个目录操作。
