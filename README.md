...
- 文件管理首页会聚合展示所有已保存账号下的 bucket / 根目录，并在“来源”列明确标出账号名；列表和网格只要存在总额度就显示容量进度条。WebDAV / 百度网盘按服务端已用容量填充并可悬浮查看详情；仅设置自定义总额度的 S3 显示中性空轨道和“用量未知”，不会误报为已用 0。桶设置中的自定义容量只覆盖总容量，完全没有总额度时显示 `--`。列表视图可拖拽调整桶顺序（顺序会持久保存），方便直接并行管理多个上游。
