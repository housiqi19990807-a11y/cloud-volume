...
- 文件管理首页会聚合展示所有已保存账号下的 bucket / 根目录，并在“来源”列明确标出账号名；“已用 / 配额”列展示服务端已用容量、总容量和使用进度条，悬浮可查看完整详情。网格卡片使用紧凑的同单位用量文本。桶设置中的自定义容量只覆盖总容量，未设置时读取 WebDAV RFC 4331 或百度网盘账号的真实容量，不支持配额查询的后端显示 `--`。列表视图可拖拽调整桶顺序（顺序会持久保存），方便直接并行管理多个上游。
