...
- The macOS bucket mount now uses the Desktop-visible bucket directory as the real WebDAV mount point again, and seeds local-only system dot paths such as `.TemporaryItems`, `.Trashes`, and `.fseventsd` so Finder and Archive Utility can create writable temporary content without pushing those macOS-private files back to S3.
- The macOS mount overlay also pre-creates Finder trash folders `.Trash`, `.Trash-<uid>`, and `.Trashes/<uid>` with volume-style permissions so local trash moves have the writable staging paths Finder expects before it talks to WebDAV.
...
- Deletes now use an app-level recycle bin per bucket instead of Finder trash on the mounted WebDAV volume, so files and directories are soft-deleted into a hidden bucket directory such as `.trash` and can then be restored or permanently deleted from the app.
