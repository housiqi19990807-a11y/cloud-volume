...
- 主题强调色
- **局域网 P2P 同步**（`p2p_enabled`，默认开启）：同局域网内登录同一账号的多台设备通过 mDNS 自动发现，写入完成后即时通知其他设备刷新目录缓存，无需等待远端轮询。可在「设置 → 局域网同步」开关
- **P2P 传输分块大小**（`p2p_chunk_size_mb`，默认 4 MB，范围 1-64 MB）：局域网内容直传时的分块大小
- WebDAV 登录账号与密码
