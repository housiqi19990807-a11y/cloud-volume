...
- macOS WebDAV 挂载的内容写入会先落到本地缓存，再按 quiet period 异步推送上游。Finder 为文件时间等属性发送的 `PROPPATCH` 元数据探测不会触发文件内容下载或重复上传；SFTP 上游的上传进度、成功和失败也会及时反映到传输队列，不会在实际同步完成后继续停留在“等待同步”。
