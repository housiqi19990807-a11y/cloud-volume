...
- 新 Windows 机器可以先运行 `powershell -ExecutionPolicy Bypass -File .\scripts\setup_windows_dev.ps1` 一键准备开发环境。脚本会通过 `winget` 安装/校验 Git、Go、Visual Studio 2022 Build Tools、MSYS2；如果本机没有 `winget`，VS Build Tools 和 MSYS2 会回退到官方安装器直链静默安装。脚本会把 Flutter stable clone 到默认的 `$HOME\dev\flutter`，安装 MSYS2 UCRT64 `gcc/g++`，写入 `FLUTTER_ROOT` / `BRIDGE_CC` / `BRIDGE_CXX` 和用户 `PATH`。
