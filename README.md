...
- 文件管理：桶列表、目录浏览、列表/网格视图、右键操作、搜索、多选、批量下载/删除；新增或初始化账号时可配置映射到文件管理页的桶名称，WebDAV 目录会按当前层级探测只读权限并限制写入入口，根目录只读但子目录可写时会以当前子目录权限为准，桌面端还支持把本地文件拖入文件列表上传、复制本地文件后在列表中粘贴上传，以及复制远端文件后粘贴到 Finder / Explorer 等本地目录。
...
- 回收站：应用级软删除、全局回收站 / 桶级回收站、行内恢复与彻底删除操作、批量删除、一键清空、分页与无限滚动。
