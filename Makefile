...
bridge-macos:
	@mkdir -p $(BRIDGE_DIR)
	$(BRIDGE_GO_ENV) go build -buildmode=c-shared -ldflags "$(BRIDGE_LDFLAGS)" -o $(MACOS_BRIDGE_OUT) ./bridge

bridge-linux:
	@mkdir -p $(BRIDGE_DIR)
	$(BRIDGE_GO_ENV) go build -buildmode=c-shared -ldflags "$(BRIDGE_LDFLAGS)" -o $(LINUX_BRIDGE_OUT) ./bridge
